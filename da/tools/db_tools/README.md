...
**Note:** Other database adapters (MySQL, Snowflake, StarRocks, etc.) are available as separate adapter packages. Install them separately as needed, e.g., `pip install DA-mysql`.
...
```python
from da.tools.db_tools import DBManager, SQLiteConnector
...
from da.schemas.node_models import ExecuteSQLInput
...
New database adapters should be created as separate adapter packages (e.g., `DA-newdb`). This keeps the core package lightweight while allowing users to install only the database adapters they need.

See the [DA-adapters](https://github.com/DA-ai/DA-adapters) repository for existing adapter implementations and contribution guidelines.

1. **Create a new adapter package** following the structure in the DA-adapters repository:
...
```python
from da.tools.db_tools.base import BaseSqlConnector
from da.schemas.node_models import ExecuteSQLInput, ExecuteSQLResult
...
```toml
[project.entry-points."DA.adapters"]
newdb = "DA_newdb:register"
```
...
- **Query Errors**: Standardized error codes and messages through `DaException`
