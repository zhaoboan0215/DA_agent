...
from da.tools.db_tools.config import DuckDBConfig
...
def _metadata_names(_type: str) -> _DBMetadataNames:
    if _type not in METADATA_DICT:
        raise DaException(ErrorCode.COMMON_FIELD_INVALID, f"Invalid type `{_type}` for Database table type")
...
class DuckdbConnector(BaseSqlConnector, SchemaNamespaceMixin, MigrationTargetMixin):
    ...
    def _handle_exception(self, e: Exception, sql: str = "") -> DaException:
        """Handle DuckDB exceptions and map to appropriate DA ErrorCode."""
    ...
    def execute_content_set(self, sql_query: str) -> ExecuteSQLResult:
        ...
        try:
            ...
            from da.utils.sql_utils import parse_context_switch
    ...
    def get_schema(
        ...
        try:
            ...
            if "error_message" in e.message_args:
                ...
                message = e.message
            raise DaException(ErrorCode.DB_QUERY_METADATA_FAILED, message=message)
        except Exception as e:
            raise DaException(
