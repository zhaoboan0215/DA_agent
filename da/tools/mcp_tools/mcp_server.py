...
def find_mcp_directory(mcp_name: str) -> str:
    ...
    for path in sys.path:
        if "site-packages" in path:
            DA_mcp_path = Path(path) / "mcp" / mcp_name
            if DA_mcp_path.exists():
                logger.info(f"Found MCP directory via sys.path: {DA_mcp_path}")
                return str(DA_mcp_path)

    raise FileNotFoundError(
        f"MCP directory '{mcp_name}' not found in development mcp directory or installed da-mcp package"
    )
