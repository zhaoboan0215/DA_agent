...
"""Scheduler tools for submitting and managing jobs via DA scheduler adapters."""
...
from da.tools import BaseTool
from da.tools.func_tool.base import FuncToolListResult, FuncToolResult, trans_to_function_tool
...
class SchedulerTools(BaseTool):
    ...
    def _get_adapter(self):
        ...
        try:
            from DA_scheduler_core.registry import SchedulerAdapterRegistry
        except ImportError as exc:
            raise DaException(
                ErrorCode.COMMON_MISSING_DEPENDENCY,
                message_args={},
                message="DA-scheduler-core is required for scheduler tools. "
                "Install it with: pip install DA-scheduler-core",
            ) from exc
        ...
        # (``{dags_folder_root}/{project_name}/``) so multiple DA
        # instances writing DAG files to the same Airflow cluster never
        # collide on disk. It does NOT auto-derive a ``dag_id_prefix``
        # anymore (the two concerns were split in DA-scheduler-airflow
        # 0.2.0), so reads aren't silently filtered by the DA workspace.
        # Auto-injecting the DA workspace identifier is safe: it stays
    ...
    def submit_sql_job(
        ...
        """
        try:
            from DA_scheduler_core.models import SchedulerJobPayload
        except ImportError as exc:
            return FuncToolResult(success=0, error=f"DA-scheduler-core not installed: {exc}")
        ...
        except (ImportError, ValueError, DaException) as exc:
