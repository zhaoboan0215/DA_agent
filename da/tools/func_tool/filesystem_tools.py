...
from da.tools.func_tool import FuncToolResult
...
class FilesystemFuncTool(BaseTool):
    ...
    * ``HIDDEN`` — return a ``File not found`` style error for reads/writes
      and prune the subtree in walks, so ``.da/sessions`` etc. stay
    ...
    * ``EXTERNAL`` — proceed at the tool level; the ``PermissionHooks`` layer
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._DA_home = Path(DA_home).expanduser().resolve(strict=False) if DA_home else None
    ...
    def _not_found(self, resolved: ResolvedPath) -> FuncToolResult:
        ...
        "hidden by policy" — leaking that ``.da/sessions`` exists would
    ...
    def _walk_files(self, seed: ResolvedPath, include_pattern: str = "") -> Iterator[Path]:
        ...
        * ``HIDDEN`` prune: every entry is classified; ``HIDDEN`` subtrees
          are skipped entirely, which keeps ``.da/sessions`` etc. invisible
        ...
        def has_whitelisted_descendant(directory: Path) -> bool:
            ...
            Needed because ``.da/`` itself classifies HIDDEN even though
            ``.da/skills/`` inside it is WHITELIST — we still have to
