...
from da.schemas.action_history import (
...
if TYPE_CHECKING:
    from da.agent.node.agentic_node import AgenticNode
...
class SubAgentTaskTool:
    ...
    def _create_node(self, subagent_type: str):
        ...
        from da.agent.node.node import Node
    ...
    async def _execute_node(
        ...
        if self._parent_node and self._parent_node.proxy_tool_patterns:
            from da.tools.proxy.proxy_tool import apply_proxy_tools
    ...
    def _build_node_input(self, node, prompt: str):
        ...
        from da.agent.node.gen_sql_agentic_node import GenSQLAgenticNode
        from da.schemas.explore_agentic_node_models import ExploreNodeInput
