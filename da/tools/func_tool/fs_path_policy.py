...
def _resolve_home(DA_home: Optional[Path]) -> Path:
    """Resolve the effective ``~/.da`` root for whitelist anchors."""
    if DA_home is not None:
        return Path(DA_home).expanduser().resolve(strict=False)
    return (Path.home() / ".DA").resolve(strict=False)
    ...
    *,
    ...
    Args:
        ...
        current_node: Name of the currently executing node. Only then does
            ``{root_path}/.da/memory/{current_node}/**`` qualify as
            ``WHITELIST``. ``None`` causes the memory anchor to be dropped,
            which demotes every ``.da/memory/**`` path to ``HIDDEN``.
        DA_home: Override for ``~/.da``. Primarily a test hook; when
    ...
    home_resolved = _resolve_home(DA_home)
    ...
    # anchor so a project that happens to live under ``~/.da`` still
    # classifies ``{project_root}/.da/skills/x`` as the project's own skill
    # rather than the global one. See Decision Order step 4 in the plan.
    project_dot_da = (root_resolved / ".DA").resolve(strict=False)
    project_skills = (project_dot_da / "skills").resolve(strict=False)
    project_memory_node: Optional[Path] = None
    if current_node:
        project_memory_node = (project_dot_da / "memory" / current_node).resolve(strict=False)
    ...
    if any(_is_relative_to(resolved, anchor) for anchor in whitelist_anchors):
        ...
        if _is_relative_to(resolved, root_resolved):
            ...
            # ``home_resolved`` already *is* the ``.DA`` directory, so we
            # rebuild the display with the canonical ``~/.da/`` prefix the
            # LLM can feed back unambiguously. Without this we would show
            # ``~/skills/foo`` and lose which home we meant.
            display = "~/.da/" + resolved.relative_to(home_resolved).as_posix()
        else:
            display = str(resolved)
    elif _is_relative_to(resolved, project_dot_da):
...
def whitelist_anchors(
    *,
    ...
    home_resolved = _resolve_home(DA_home)
    project_dot_da = (root_resolved / ".DA").resolve(strict=False)
    anchors = [(project_dot_da / "skills").resolve(strict=False)]
    if current_node:
        anchors.append((project_dot_da / "memory" / current_node).resolve(strict=False))
...
def build_walk_patterns(
    *,
    ...
    subtrees under ``.da/`` stay visible.
    ...
    excludes = [".DA", ".da/**"]
    re_includes = [".da/skills/**"]
    if current_node:
        re_includes.append(f".da/memory/{current_node}/**")
