...
This package provides a unified abstraction layer for semantic layer services (MetricFlow, dbt Semantic Layer, Cube, etc.), following the adapter pattern similar to `da/tools/db_tools/`.
...
```python
from da.tools.semantic_tools import BaseSemanticAdapter
...
```python
from da.tools.semantic_tools import semantic_adapter_registry
...
```python
from da.tools.semantic_tools import SemanticAdapterConfig  # Base config
```
...
```python
from da.tools.semantic_tools import SemanticStorageManager
...
```python
from da.tools.func_tool.semantic_tools import SemanticTools
...
Create a new package in the `DA-semantic-adapter` repository.

**Repository Structure** (following `DA-db-adapters` pattern):

```
DA-semantic-adapter/              # Repository root
├── DA-semantic-metricflow/       # Each adapter project directory
│   ├── pyproject.toml               # Package config for this adapter
│   └── DA_semantic_metricflow/   # Python package
...
│       │   └── test_adapter.py
│       └── examples/
└── DA_semantic_cube/             # Future: Cube adapter
    ├── pyproject.toml
    └── DA_semantic_cube/
        └── ...
```
...
pip install -e ../DA-semantic-adapter/DA-semantic-metricflow
```

### Step 2: Implement Adapter Class

```python
# DA_metricflow/adapter.py
import asyncio
...
class MetricFlowAdapter(BaseSemanticAdapter):
    ...
    async def query_metrics(
        ...
        # Add dimensions (use --group-by for DA-metricflow)
        if dimensions:
    ...
    async def validate_semantic(self) -> ValidationResult:
        ...
        Note: DA-metricflow automatically finds semantic model files at:
...
```python
# DA_semantic_metricflow/config.py
...
**Note**: For DA-metricflow, the semantic model files are automatically located at:
- `{agent.home}/semantic_models/{datasource}/`
- `agent.home` is read from `agent.yml` config (defaults to `~/.da`)

### Step 4: Create Registration Function

```python
# DA_semantic_metricflow/__init__.py
from DA_semantic_metricflow.adapter import MetricFlowAdapter
from DA_semantic_metricflow.config import MetricFlowConfig

def register():
    """Register MetricFlow adapter with Da semantic adapter registry."""
    # Import at runtime to avoid circular dependencies
    from da.tools.semantic_tools.registry import semantic_adapter_registry
...
```toml
# pyproject.toml (in DA-semantic-metricflow directory)
[project]
name = "DA-semantic-metricflow"
version = "0.1.0"
description = "MetricFlow adapter for Da semantic layer"
dependencies = [
    "pydantic>=2.0.0",
]

[project.entry-points."DA.semantic_adapters"]
metricflow = "DA_semantic_metricflow:register"
...
include = ["DA_semantic_metricflow*"]
```

### Step 6: Install and Use

```bash
# Install specific adapter from DA-semantic-adapter repository
pip install -e ../DA-semantic-adapter/DA-semantic-metricflow

# Verify installation
pip show DA-semantic-metricflow
```
...
# Or let Da auto-create config from datasource
...
- Adapters auto-register via `DA.semantic_adapters` entry point
...
The MetricFlow adapter uses DA-metricflow CLI which automatically finds semantic model files:

1. **Datasource parameter**: Required, used to locate files
2. **Model path resolution**: `{agent.home}/semantic_models/{datasource}/`
   - `agent.home` is read from `agent.yml` (defaults to `~/.da`)
...
3. **Agent config lookup priority**:
   ...
   - `~/.da/conf/agent.yml` (home directory)

### Example agent.yml

```yaml
agent:
  home: /Users/myuser/.da  # Optional, defaults to ~/.da
...
Use the predefined error codes from `DA.utils.exceptions`:
...
- **Architecture Pattern**: [da/tools/db_tools/](../db_tools/) - Database adapter pattern
- **Storage Integration**: [da/storage/semantic_model/](../../storage/semantic_model/), [da/storage/metric/](../../storage/metric/)
- **Example Usage**: [da/agent/node/gen_metrics_agentic_node.py](../../agent/node/gen_metrics_agentic_node.py)
