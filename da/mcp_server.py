...
Da's database and context search tools as MCP-compatible tools.
...
Usage:
    ...
    python -m da.mcp_server --dynamic
    python -m da.mcp_server --dynamic --host 0.0.0.0 --port 8000

    # Run dynamic server with SSE transport
    python -m da.mcp_server --dynamic --transport sse
    ...
    uv run da-mcp --datasource demo
    uv run da-mcp --datasource demo --transport stdio

    # Run with uvx (after installing from PyPI)
    uvx --from da-agent da-mcp --datasource demo
    uvx --from da-agent da-mcp --datasource demo --transport stdio

    # Run with HTTP streamable mode (default)
    python -m da.mcp_server --datasource demo
    python -m da.mcp_server --datasource demo --host 0.0.0.0 --port 8000

    # Run with HTTP SSE mode
    python -m da.mcp_server --datasource demo --transport sse --port 8000

    # Run with stdio (for Claude Desktop)
    python -m da.mcp_server --datasource demo --transport stdio
    ...
    # For Claude Desktop config (claude_desktop_config.json):
    {
        "mcpServers": {
            "da": {
                "command": "uvx",
                "args": ["--from", "da-agent", "da-mcp", "--datasource", "demo", "--transport", "stdio"]
            }
        }
    }
    ...
    # Alternative config using python directly:
    {
        "mcpServers": {
            "da": {
                "command": "python",
                "args": ["-m", "da.mcp_server", "--datasource", "demo", "--transport", "stdio"]
            }
        }
    }
...
from da.tools.func_tool.reference_template_tools import ReferenceTemplateTools
from da.utils.loggings import configure_logging, get_logger
...
from da.utils.mcp_decorators import get_tool_registry  # noqa: E402
...
class LightweightDynamicMCPServer:
    ...
    def __init__(self, config_path: Optional[str] = None, max_cache_size: Optional[int] = 64):
        ...
        self.mcp = FastMCP(
            name="da",
            instructions=(
                "Da is a data engineering agent that provides tools for querying databases, "
    ...
    def _register_tools(self):
        ...
        from da.utils.mcp_decorators import register_dynamic_tools
    ...
    def create_asgi_app(self, transport: Literal["http", "sse"] = "http"):
        ...
        async def lifespan(_app):
            logger.info(f"Starting Da MCP Server (Lightweight Dynamic Mode, transport={current_transport})")
            ...
            logger.info("Da MCP Server stopped")
...
class DaMCPServer:
    """
    MCP Server that wraps Da's database and context search tools.
    ...
    def _register_tools(self):
        ...
        from da.utils.mcp_decorators import register_static_tools
    ...
    def run(
        ...
        """
        logger.info(f"Starting Da MCP Server (datasource={self.datasource}, transport={transport})")
    ...
    def get_sse_app(self):
        ...
        This allows mounting the MCP server in an existing FastAPI/Starlette app:
            ...
            from da.mcp_server import create_server
...
def create_server(
    ...
    """
    Factory function to create a DaMCPServer instance.
    ...
    Returns:
        Configured DaMCPServer instance
    """
    return DaMCPServer(
        datasource=datasource,
        sub_agent=sub_agent,
...
def run_dynamic_server(
    ...
    print(f"\n{'=' * 60}")
    print(f"  Da MCP Server (Dynamic Mode, transport={transport})")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Da MCP Server - Expose DA tools via Model Context Protocol",
...
Examples:
    ...
    python -m da.mcp_server --datasource demo --config /path/to/agent.yml
