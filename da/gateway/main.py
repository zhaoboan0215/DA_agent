...
"""
CLI entry point for the Da Gateway IM gateway.
...
def _run_gateway(args: argparse.Namespace) -> None:
    """Load config and start the Da Gateway (blocking)."""
    ...
    from da.gateway.runtime import DaGateway
    ...
    logger.info("Starting Da Gateway...")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Da Gateway — IM Channel Gateway")
    parser.add_argument("-v", "--version", action="version", version=f"Da Gateway {__version__}")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        help="Default datasource (default: DA_DATASOURCE env or 'default')",
    )
    ...
    parser.add_argument(
        "--pid-file",
        type=str,
        help=f"PID file path (default: ~/.da/run/{_SERVICE_NAME}.pid)",
    )
    parser.add_argument(
        "--daemon-log-file",
        type=str,
        help=f"Daemon log file path (default: ~/.da/logs/{_SERVICE_NAME}.log)",
    )
...
def main() -> None:
    """Main entry point for starting the Da Gateway."""
    ...
    if args.daemon:
        ...
        log_dir = str(log_file.parent)
        configure_logging(args.debug, log_dir=log_dir, console_output=False)
        logger.info(f"Starting Da Gateway (daemon) on {args.host}:{args.port} | Debug: {args.debug}")
    ...
    logger.info(f"Starting Da Gateway on {args.host}:{args.port}")
    _run_gateway(args)
