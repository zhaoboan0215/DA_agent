...
from da.api.auth import load_auth_provider
from da.api.deps import init_deps
from da.api.services.da_service_cache import DaServiceCache
from da.configuration.agent_config_loader import load_agent_config, parse_config_path
...
from da.storage.task import TaskStore
...
class DaAPIService:
    """Main service class for Da Agent API."""
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.warning(
                ...
                "until a model is set up via 'da' CLI + /model command."
            )
...
async def lifespan(app: FastAPI):
    ...
    service = DaAPIService(args)

    # Startup
    await service.initialize()

    # Initialize plugin-based auth and service cache for new API routes
    datasource = getattr(args, "datasource", None) or os.getenv("DA_DATASOURCE", "default")
    ...
    service_cache = DaServiceCache(max_size=128)
    init_deps(
        auth_provider,
        service_cache,
    ...
    logger.info("Da API Service started")
    yield
    # Shutdown
    logger.info("Da API Service shutting down")
...
def create_app(agent_args: argparse.Namespace) -> FastAPI:
    ...
    app = FastAPI(
        title="Da Agent API",
        description="FastAPI service for Da Agent workflow execution",
        version="1.0.0",
        lifespan=lifespan,
    )
    ...
    # Add CORS middleware
    cors_origins_env = os.getenv("DA_CORS_ORIGINS", "*")
    ...
    _route_modules = [
        ("da.api.routes.chat_routes", "chat"),
        ("da.api.routes.cli_routes", "cli"),
        ("da.api.routes.database_routes", "database"),
        ("da.api.routes.table_routes", "table"),
        ("da.api.routes.explorer_routes", "explorer"),
        ("da.api.routes.config_routes", "config"),
        ("da.api.routes.models_routes", "models"),
        ("da.api.routes.mcp_routes", "mcp"),
        ("da.api.routes.kb_routes", "kb"),
        ("da.api.routes.agent_routes", "agent"),
        ("da.api.routes.visualization_routes", "visualization"),
        ("da.api.routes.tool_routes", "tool"),
        ("da.api.routes.success_story_routes", "success_story"),
    ]
    import importlib
    ...
    async def root():
        ...
        return {"message": "Da Agent API", "version": "1.0.0", "docs": "/docs", "health": "/health"}
...
# Configuration via environment variables: DA_CONFIG, DA_DATASOURCE, DA_OUTPUT_DIR, DA_LOG_LEVEL
_config_file = os.getenv("DA_CONFIG", "")  # Empty string uses default resolution
try:
...
_datasource = os.getenv("DA_DATASOURCE", "default")
_output_dir = os.getenv("DA_OUTPUT_DIR", "./output")
_log_level = os.getenv("DA_LOG_LEVEL", "INFO")
