...
"""
Da Agent FastAPI server startup script.

Single entry point for both the ``da-api`` console script and
``python -m da.api.main``. Supports foreground and daemon
...
from da.configuration.agent_config_loader import parse_config_path
...
def _default_paths(config_path: str = "") -> Tuple[Path, Path]:
    ...
    from da.configuration.agent_config_loader import get_agent_home
    from da.utils.path_manager import DaPathManager

    path_manager = DaPathManager(get_agent_home(config_path))
    pid_file = path_manager.pid_file_path("da-agent-api")
    log_file = path_manager.logs_dir / "da-agent-api.log"
...
def _run_server(args: argparse.Namespace, agent_args: argparse.Namespace) -> None:
    from da.api.service import create_app
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Da Agent FastAPI Server")
    parser.add_argument("-v", "--version", action="version", version=f"Da API {__version__}")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Agent configuration file (default: ./conf/agent.yml > ~/.da/conf/agent.yml)",
    )
    parser.add_argument(
        "--datasource",
        type=str,
        default=os.getenv("DA_DATASOURCE", "default"),
        help="Datasource identifier (default: DA_DATASOURCE env or 'default')",
    )
    parser.add_argument(
        "--output-dir",
        dest="output_dir",
        type=str,
        default=os.getenv("DA_OUTPUT_DIR", "./output"),
    ...
    parser.add_argument(
        ...
        dest="log_level",
        type=str,
        default=os.getenv("DA_LOG_LEVEL", "INFO"),
        choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"],
        help="Log level (default: INFO or DA_LOG_LEVEL env var)",
    )
    ...
    parser.add_argument(
        "--pid-file",
        type=str,
        help="PID file path (default: ~/.da/run/da-agent-api.pid)",
    )
    parser.add_argument(
        "--daemon-log-file",
        type=str,
        help="Daemon log file path (default: ~/.da/logs/da-agent-api.log)",
    )
    return parser


def main():
    """Main entry point for starting the Da Agent API server."""
    ...
    os.environ["DA_CONFIG"] = config_path
    os.environ["DA_DATASOURCE"] = args.datasource
    os.environ["DA_OUTPUT_DIR"] = args.output_dir
    os.environ["DA_LOG_LEVEL"] = args.log_level
    ...
    if args.daemon:
        ...
        configure_logging(args.debug, log_dir=str(log_file.parent), console_output=False)
        logger.info(
            f"Starting Da Agent API server (daemon) on {args.host}:{args.port} | "
    ...
    logger.info(f"Starting Da Agent API server on {args.host}:{args.port}")
