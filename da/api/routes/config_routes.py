...
from da.api.deps import AppContextDep, ServiceDep
from da.api.models.base_models import Result
from da.configuration.agent_config import _SAFE_NAME_RE, DbConfig, load_model_config
...
def _probe_llm_sync(payload: Dict[str, Any]) -> None:
    ...
    module = __import__(f"da.models.{model_cfg.type}_model", fromlist=[model_class_name])
...
async def _evict_current_project(project_id: str) -> None:
    """Drop the cached DaService so the next request reloads from YAML."""
