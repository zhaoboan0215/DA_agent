...
from da.api.models.config_models import ModelInfo, ModelPricing, ModelsData
from da.cli.provider_model_catalog import load_cache_fetched_at, load_cached_model_details
...
def _model_spec(catalog: Dict[str, Any], slug: str) -> Dict[str, Any]:
    ...
    ``da/conf/providers.yml:154-155``: the longest spec key that is a prefix
...
async def list_models(svc: ServiceDep) -> Result[ModelsData]:
    ...
    Data priority per-model:
      1. OpenRouter cache (``~/.da/cache/openrouter_models.json``) — richest.
