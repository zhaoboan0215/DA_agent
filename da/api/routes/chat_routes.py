...
ChatService via DaService. Routes are thin wrappers that handle HTTP
concerns only.

Streaming endpoints use the project-scoped ChatTaskManager (via
DaService.task_manager) to run the agentic loop in a background
...
# in ``DA.api.constants``. Keep this list in sync with the dispatch branches
