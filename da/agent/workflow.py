...
from da.schemas.node_models import Context, SQLContext, SqlTask
...
class Workflow:
    ...
    def get_last_sqlcontext(self) -> SQLContext:
        ...
        Raises:
            DaException: If no SQL context is available
        """
        if len(self.context.sql_contexts) > 0:
            return self.context.sql_contexts[-1]
        raise DaException(ErrorCode.NODE_NO_SQL_CONTEXT)
