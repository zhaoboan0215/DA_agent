...
editing, and scaffolding Da skills. It exposes filesystem (read+write),
...
class SkillCreatorAgenticNode(AgenticNode):
    ...
    def _setup_full_filesystem_tools(self):
        ...
        ``.da/skills/`` and ``~/.da/skills/`` are WHITELIST (writable),
        the rest of the project tree is INTERNAL (also writable by this node),
        ``.da/`` internals other than skills are HIDDEN (invisible), and
    ...
    def _get_system_prompt(
        ...
        if self.agent_config:
            ...
            if skills_config and hasattr(skills_config, "directories"):
                ...
                skill_directories = ["./.da/skills", "~/.da/skills"]
