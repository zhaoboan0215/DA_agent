...
"""
Agentic Node Architecture for Da-agent.
...
from da.cli.execution_state import ExecutionInterrupted, InteractionBroker, InterruptController
...
class AgenticNode(Node):
    ...
    def __init__(
        ...
        from da.utils.memory_loader import has_memory
    ...
    def _inject_memory_context(self, base_prompt: str, override_node_name: Optional[str] = None) -> str:
        ...
        """
        from da.utils.memory_loader import get_memory_dir, load_memory_context
    ...
    def _setup_ask_user_tool(self):
        ...
        """
        try:
            from da.tools.func_tool.ask_user_tools import AskUserTool
    ...
    def _setup_sub_agent_task_tool(self):
        ...
        try:
            from da.tools.func_tool.sub_agent_task_tool import SubAgentTaskTool
    ...
    def _get_result_class(self):
        ...
        try:
            ...
            if class_name == "ChatAgenticNode":
                from da.schemas.chat_agentic_node_models import ChatNodeResult

                return ChatNodeResult
            elif class_name == "GenSQLAgenticNode":
                from da.schemas.gen_sql_agentic_node_models import GenSQLNodeResult

                return GenSQLNodeResult
            elif class_name == "CompareAgenticNode":
                from da.schemas.compare_node_models import CompareResult

                return CompareResult
            elif class_name == "ExploreAgenticNode":
                from da.schemas.explore_agentic_node_models import ExploreNodeResult
    ...
    async def get_last_turn_usage(self) -> Optional[TokenUsage]:
        ...
        from da.schemas.token_usage import TokenUsage as _TokenUsage
    ...
    def _make_filesystem_tool(self, **kwargs):
        ...
        classify ``.da/memory/{current_node}/**`` as a whitelist subtree
        ...
        """
        from da.tools.func_tool import FilesystemFuncTool

        root_path = kwargs.pop("root_path", None) or self._resolve_workspace_root()
        da_home = kwargs.pop("da_home", None)
        if da_home is None and self.agent_config is not None:
            path_manager = getattr(self.agent_config, "path_manager", None)
            if path_manager is not None:
                try:
                    da_home = str(path_manager.da_home)
                except Exception:
                    da_home = None
