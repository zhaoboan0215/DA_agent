...
from da.schemas.compare_node_models import CompareInput, CompareResult
...
class CompareNode(Node):
    ...
    def _execute_compare(self) -> CompareResult:
        ...
        if not isinstance(self.input, CompareInput):
            raise DaException(ErrorCode.COMMON_VALIDATION_FAILED, "Input must be a CompareInput instance")

        if not self.model:
            raise DaException(ErrorCode.COMMON_VALIDATION_FAILED, "Model is not initialized for CompareAgenticNode")
