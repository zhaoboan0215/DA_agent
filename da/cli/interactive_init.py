...
"""
Interactive initialization command for Da Agent.
...
from da.cli.cli_styles import print_error
from da.cli.init_util import detect_db_connectivity
from da.cli.provider_auth_flows import configure_claude_subscription, configure_codex_oauth
from da.configuration.agent_config import AgentConfig
from da.configuration.project_config import ProjectOverride, ProjectTarget, save_project_override
from da.utils.loggings import configure_logging, get_logger, print_rich_exception
from da.utils.path_manager import get_path_manager
from da.utils.path_utils import safe_rmtree
from da.utils.resource_utils import copy_data_file, read_data_file_text

logger = get_logger(__name__)


class InteractiveInit:
    """Interactive initialization wizard for Da Agent."""
    ...
    def __init__(self, user_home: Optional[str] = None):
        ...
        # so we fall back to the context-local path manager / default ~/.da here.
        ...
        # ``{cwd}/.da/config.yml`` so each project can pin its own model
        ...
        # Staged project-level target; written to ``./.da/config.yml`` at
    ...
    def run(self) -> int:
        ...
        try:
            self.console.print("\n[bold cyan]Welcome to Da Init 🎉[/bold cyan]")
    ...
    def _configure_llm(self) -> bool:
        ...
        Result shape (written to ``self.config``):
          ...
          - ``self._pending_target`` — ``ProjectTarget(provider, model)``,
            flushed to ``{cwd}/.da/config.yml`` in ``_save_configuration``.
    ...
    def _configure_workspace(self) -> bool:
        ...
        default_workspace = str(self.user_home / ".DA" / "workspace")
        ...
        self.config["agent"]["storage"]["base_path"] = str(self.user_home / ".DA" / "data")
    ...
    def _save_configuration(self) -> bool:
        """Persist credentials to ``~/.da/conf/agent.yml`` and target to
        ``./.da/config.yml``.
    ...
    def _display_completion(self):
        ...
        self.console.print(f"\nYou are ready to run `da-cli --datasource {self.datasource_name}` 🚀")
        self.console.print("\nCheck the document at https://docs.DA.ai/ for more details.")
    ...
    def _test_llm_connectivity(self) -> tuple[bool, str]:
        ...
        try:
            ...
            module = __import__(f"da.models.{model_type}_model", fromlist=[class_name])
...
def init_metadata_and_log_result(datasource_name: str, config_path: str, console: Console):
    from da.configuration.agent_config_loader import load_agent_config
    from da.storage.schema_metadata.local_init import init_local_schema
...
def do_init_sql_and_log_result(
    ...
    from da.utils.stream_output import StreamOutputManager
