...
"""
DA-CLI REPL (Read-Eval-Print Loop) implementation.
...
from da.cli.agent_commands import AgentCommands
from da.cli.autocomplete import (
...
from da.cli.bi_dashboard import BiDashboardCommands
from da.cli.chat_commands import ChatCommands
from da.cli.context_commands import ContextCommands
from da.cli.language_commands import LanguageCommands
from da.cli.list_selector_app import ListItem, ListSelectorApp
from da.cli.metadata_commands import MetadataCommands
from da.cli.model_commands import ModelCommands
from da.cli.service_commands import ServiceCommands
from da.cli.slash_registry import GROUP_ORDER, GROUP_TITLES, iter_visible, lookup
from da.cli.status_bar import StatusBarProvider
from da.cli.sub_agent_commands import SubAgentCommands
from da.cli.tui import DaApp, tui_enabled
from da.cli.tui.app import EXIT_SENTINEL
from da.configuration.agent_config_loader import configuration_manager, load_agent_config
from da.schemas.action_history import ActionHistory, ActionHistoryManager, ActionRole, ActionStatus
from da.schemas.node_models import SQLContext
from da.tools.db_tools.db_manager import db_manager_instance
from da.utils.constants import HIDDEN_SYS_SUB_AGENTS, SYS_SUB_AGENTS, DBType, SQLType
from da.utils.exceptions import setup_exception_handler
...
class DaCLI:
    """Main REPL for the Da CLI application."""
    ...
    def __init__(self, args, interactive: bool = True):
        ...
        from da.utils.path_manager import set_current_path_manager
        ...
        # disabled via ``DA_TUI=0`` as an escape hatch.
        self._use_tui = self.interactive and tui_enabled()
        self.tui_app: Optional[DaApp] = None
        ...
        from da.cli.datasource_commands import DatasourceCommands
    ...
    def _get_prompt_text(self):
        ...
        The Da brand, plan mode, and current agent are now rendered by the
    ...
    def _init_tui_app(self) -> None:
        """Create the persistent ``DaApp`` and register REPL bindings."""
        ...
        def _s_tab(event):  # noqa: ANN001
            ...
            """
            from da.cli.tui.console_bridge import run_in_terminal_sync
        ...
        def _c_c(event):  # noqa: ANN001
            ...
            Overrides the default DaApp binding because the TUI needs a
    ...
    def create_combined_completer(self):
        ...
        from da.cli.autocomplete import SQLCompleter
    ...
    def _background_init_agent(self):
        ...
        try:
            ...
            from da.agent.agent import Agent
    ...
    # there is nothing to rebuild on a switch — see ``da/agent/agent.py``
    ...
    def _cmd_mcp(self, args):
        from da.cli.mcp_commands import MCPCommands

        MCPCommands(self).cmd_mcp(args)

    def _cmd_skill(self, args):
        from da.cli.skill_commands import SkillCommands
    ...
    def _cmd_help(self, args: str):
        ...
        CMD_WIDTH = 30
        lines: list[str] = ["[green]DA-CLI Help[/]\n"]
    ...
    def _build_banner_panel(self) -> Panel:
        ...
        if use_art:
            body.add_row(Text(DA_BANNER_TEXT, style="bold"))
        else:
            body.add_row(Text(f"DA v{__version__}", style="bold"))
        body.add_row(Text(""))
