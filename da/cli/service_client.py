...
def _build_bi_tool(agent_config: "AgentConfig", service_name: str) -> Any:
    from da.tools.func_tool.bi_tools import BIFuncTool
...
def _probe_scheduler_adapter(agent_config: "AgentConfig", service_name: str) -> bool:
    """True when both ``DA-scheduler-core`` and the platform adapter
    ...
    ``DA_scheduler_core`` imports is not enough — core is the base
    framework; the per-platform adapter (``DA-scheduler-airflow`` /
    ``DA-scheduler-dolphinscheduler`` / ...) is what actually
    ...
    # ``DA-scheduler-core`` version. Try them in order before giving up.
...
class ServiceClientRegistry:
    ...
    CLI's lowercased command tokens (see ``DaCLI._parse_command``).
    """
    ...
    def adapter_available(self, service_name: str) -> bool:
        ...
        (``DA-bi-<platform>``, ``DA-scheduler-core``, ``DA-semantic-<type>``)
