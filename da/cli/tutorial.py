...
from da.cli.init_util import init_metrics, init_semantic_model
from da.cli.interactive_init import parse_subject_tree
...
from da.utils.loggings import get_logger, print_rich_exception
from da.utils.sub_agent_manager import SubAgentManager
...
class BenchmarkTutorial:
    ...
    def _ensure_files(self):
        ...
        from da.cli.interactive_init import copy_data_file
    ...
    def _ensure_config(self) -> bool:
        if self.config_path and not Path(self.config_path).expanduser().resolve().exists():
            self.console.print(
                ...
                "please check it or run `da-agent init` first."
            )
            return False
        ...
        if (
            ...
            services_config["datasources"]["california_schools"] = {
                ...
                "uri": "~/.da/benchmark/california_schools/california_schools.sqlite",
            }
    ...
    def run(self):
        try:
            self.console.print("[bold cyan]Welcome to Da benchmark data preparation tutorial 🎉[/bold cyan]")
            self.console.print(
            ...
            from da.cli.interactive_init import init_metadata_and_log_result, overwrite_sql_and_log_result

            self.console.print("[bold yellow][2/6] Initialize Metadata using command: [/bold yellow]")
            self.console.print(
                f"    [bold green]da-agent[/] [bold]bootstrap-kb --config {self.config_path} "
            ...
            with self.console.status("Sub-Agents Building...") as status:
                ...
                self.console.print(
                    ...
                    "  1. Conduct multi-turn conversations in the CLI via `/DA_schools <your question>` or  "
                    "`/DA_schools_context <your question>`\n"
                    "  2. Use them in benchmark by running the command "
                    "`da-agent benchmark --workflow DA_schools`.[/]"
                )
    ...
    def add_sub_agents(self):
        ...
        manager.save_agent(
            ...
            previous_name="DA_schools",
        )
        self.console.print("  ✅ Sub-agent `DA_schools` have been added. It can work using database tools.")

        manager.save_agent(
            SubAgentConfig(
                system_prompt="DA_schools_context",
                prompt_version="1.0",
                prompt_language="en",
                agent_description="",
                rules=[],
            ...
            previous_name="DA_schools_context",
        )
        self.console.print(
            "  ✅ Sub-agent `DA_schools_context` have been added. "
    ...
    def add_workflows(self):
        ...
        config_manager.update_item(
            "workflow",
            value={
                "DA_schools": ["DA_schools", "execute_sql", "output"],
                "DA_schools_context": ["DA_schools_context", "execute_sql", "output"],
            },
            delete_old_key=False,
            save=True,
        )
        self.console.print("  ✅ Workflow `DA_schools` and `DA_schools_context` have been added.")
