...
from da.cli.interactive_init import ReferenceSqlStreamHandler
from da.configuration.agent_config import AgentConfig, DashboardConfig
from da.configuration.agent_config_loader import configuration_manager
from da.schemas.agent_models import ScopedContext, SubAgentConfig
...
from da.utils.reference_paths import quote_path_segment
from da.utils.sql_utils import metadata_identifier, parse_table_name_parts
...
class BiDashboardCommands:
    def __init__(
        self, agent_config: AgentConfig | "DaCLI", console: Optional[Console] = None, force: bool = False
    ) -> None:
        self.cli: Optional["DaCLI"] = None
    ...
    def _prompt_options(self) -> DashboardCliOptions:
        ...
        if not platforms:
            raise ValueError("No BI adapter implementations found. Install one with: pip install da-agent[bi]")
    ...
    def _create_adapter(self, options: DashboardCliOptions) -> BIAdapterBase:
        ...
        if adapter_cls is None:
            raise ValueError(
                f"Unsupported platform '{options.platform}'. Install it with: pip install DA-bi-{options.platform}"
            )
        return adapter_cls(
    ...
    def _gen_metrics(self, sqls: List[SelectedSqlCandidate], platform: str, dashboard: DashboardInfo) -> List[str]:
        ...
        if files := metrics_result.get("semantic_models", []):
            ...
            from da.cli.generation_hooks import resolve_kb_sandbox_path
    ...
    def _validate_semantic_model(self) -> bool:
        ...
        try:
            ...
            if not semantic_tools.adapter:
                self.console.log(
                    "[red]Semantic adapter not available. Install with: pip install DA-semantic-metricflow[/]"
                )
                return False
