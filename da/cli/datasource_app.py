...
Follows the same architecture as :mod:`DA.cli.model_app`:

* One Application owns ``stdin`` for the entire interaction.
* The outer TUI suspends via :meth:`DaApp.suspend_input` once.
...
from da.cli.cli_styles import CLR_CURRENT, CLR_CURSOR
...
class DatasourceApp:
    ...
    def _load_db_types(self) -> None:
        ...
        for t in not_installed:
            items.append((t, f"{t} (not installed \u2014 will install DA-{t})", False))
