...
"""
DA-CLI: Data engineering agent builds evolvable context for your data system.
...
from da.utils.async_utils import setup_windows_policy
...
class ArgumentParser:
    def __init__(self):
        self.parser = argparse.ArgumentParser(
            description="DA: Data engineering agent builds evolvable context for your data system"
        )
        self._setup_arguments()

    def _setup_arguments(self):
        # Add version argument
        self.parser.add_argument("-v", "--version", action="version", version=f"Da CLI {__version__}")
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=None,
            help="Path to @da/web-chatbot dist directory (for --web mode)",
        )
...
class Application:
    ...
    def run(self):
        ...
        # REPL-only: ensure ./.da/config.yml exists before anything touches
        ...
        if args.print_mode is not None:
            from da.cli.print_mode import PrintModeRunner
    ...
    def _ensure_project_config(self, args) -> None:
        """Ensure ``./.da/config.yml`` exists; create a minimal one when absent.
                ...
                raise DaException(
                    ...
                    message_args={
                        "config_error": (
                            ...
                            f"default_datasource={override.default_datasource!r} in .da/config.yml."
                        )
                    },
                )
                ...
                raise DaException(
                    ...
                    message_args={
                        "config_error": (
                            ...
                            f"Edit .da/config.yml manually or rerun in an interactive terminal."
                        )
                    },
                )
            console.print(
    ...
    def _provider_has_credentials(provider: str, raw: dict) -> bool:
        ...
        from da.configuration.agent_config import _load_provider_catalog, resolve_env
    ...
    def _run_web_interface(self, args):
        ...
        from da.cli.web import run_web_interface
...
def main():
    ...
    # Intercept 'skill' subcommand and delegate to DA.main's skill handler
    if len(sys.argv) > 1 and sys.argv[1] == "skill":
        from da.main import create_parser as create_main_parser
