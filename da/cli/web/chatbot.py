...
"""
Web Chatbot server for Da Agent.

Serves a React-based chatbot frontend (``@DA/web-chatbot`` UMD bundle)
backed by the standard Da Agent API routes.  Replaces the former
...
def _build_agent_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    ``create_app`` / ``DaAPIService``.
    """
    agent_args = argparse.Namespace(
        datasource=args.datasource,
        config=getattr(args, "config", None),
        debug=getattr(args, "debug", False),
        # Fields expected by DaAPIService but not present in CLI args
        max_steps=20,
...
def create_web_app(args: argparse.Namespace) -> FastAPI:
    ...
    if chatbot_dist:
        ...
        required_assets = [
            os.path.join(chatbot_dist, "DA-chatbot.css"),
            os.path.join(chatbot_dist, "DA-chatbot.umd.js"),
        ]
...
def run_web_interface(args: argparse.Namespace) -> None:
    ...
    """
    from da.cli.web.config_manager import get_home_from_config
    ...
    logger.info("Starting Da Web Interface...")
