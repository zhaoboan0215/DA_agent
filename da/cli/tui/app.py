...
The Da REPL historically used ``PromptSession.prompt()`` and re-rendered
the status bar as a prefix each round. During agent loops, the session was
not active and the status bar + input disappeared. :class:`DaApp` replaces
...
def tui_enabled() -> bool:
    ...
    ``DA_TUI`` environment variable is not set to a falsy value
    (``0``/``false``/``no``/``off``, case-insensitive).
    """
    env = os.environ.get("DA_TUI", "").strip().lower()
...
class DaApp:
    ...
    def __init__(
        ...
        *,
        ...
        self._executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="da-tui-worker")
        ...
        # ``completion-menu.*`` keys in ``DaCLI._build_app_style``.
    ...
    def suspend_input(self, ready_timeout: float = 2.0) -> Iterator[None]:
        ...
        if not released.wait(timeout=ready_timeout):
            ...
            try:
                ...
                pass
            raise RuntimeError("DaApp failed to release stdin within timeout")
    ...
    def run(self) -> int:
        ...
        caller *before* printing the banner (see ``DaCLI._pin_to_bottom``),
    ...
    def _build_default_key_bindings(self) -> KeyBindings:
        ...
        :attr:`key_bindings` so DaCLI can keep its existing handlers close
