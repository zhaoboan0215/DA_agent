...
-----------
  ...
  - ``/language zh --project``         — persist to .da/config.yml.
  - ``/language zh --global``          — persist to agent.yml.
  - ``/language --clear``              — remove project-level override.

The interactive path delegates to :class:`DA.cli.language_app.LanguageApp`,
...
from da.cli.cli_styles import print_success
from da.cli.language_app import LANGUAGE_CHOICES, LanguageApp, LanguageSelection
