...
The interactive path delegates to :class:`~DA.cli.datasource_app.DatasourceApp`,
...
from da.cli._cli_utils import _run_sub_application
from da.cli.cli_styles import print_error, print_info, print_success, print_warning
from da.cli.datasource_app import DatasourceApp, DatasourceSelection
from da.cli.datasource_manager import serialize_services_section
...
class DatasourceCommands:
    ...
    def _run_menu(self) -> None:
        while True:
            ...
            if selection.kind == "needs_install":
                if self._install_plugin(f"DA-{selection.db_type}"):
                    continue
                continue
            ...
            if selection.kind == "install":
                self._install_plugin(f"DA-{selection.db_type}")
                continue
    ...
    def _persist_default_datasource(self, name: str) -> None:
        """Write ``default_datasource`` to ``./.da/config.yml`` so the
        choice survives process restarts."""
        try:
            from da.configuration.project_config import ProjectOverride, load_project_override, save_project_override
    ...
    def _install_plugin(self, package: str) -> bool:
        ...
        try:
            ...
            db_type = package.removeprefix("DA-").replace("-", "_")
            try:
                module = importlib.import_module(f"DA_{db_type}")
    ...
    def _reload_runtime(self) -> None:
        try:
            from da.tools.db_tools.db_manager import _cli_cache, db_manager_instance
