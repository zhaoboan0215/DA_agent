...
The interactive path delegates to :class:`DA.cli.model_app.ModelApp`, a
...
:meth:`DA.cli.tui.app.DaApp.suspend_input` so the outer persistent
...
from da.cli.cli_styles import print_error, print_success, print_warning
from da.cli.model_app import ModelApp, ModelSelection, _display_name
...
class ModelCommands:
    ...
    def _run_oauth_flow(self, provider: str) -> bool:
        ...
        if not meta:
            ...
            return False
        from da.cli.provider_auth_flows import configure_codex_oauth
    ...
    def _persist_custom_model(self, name: str, payload: Dict[str, Any]) -> bool:
        ...
        if not name or not payload:
            ...
            return False
        try:
            from da.configuration.agent_config import load_model_config
