...
"""
Status bar provider for the Da CLI prompt.

Renders a single-line status bar above the input prompt that carries the
Da brand, current agent, model, cumulative session tokens, and context
...
class StatusBarState:
    ...
    def to_formatted_tokens(self) -> List[Tuple[str, str]]:
        ...
        tokens: List[Tuple[str, str]] = [pad, ("class:status-bar.brand", "DA")]
        if self.plan_mode:
