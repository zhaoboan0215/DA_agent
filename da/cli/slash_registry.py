...
"""Single source of truth for DA-CLI slash commands.

The registry drives three consumers:

* :attr:`DaCLI.commands` — maps ``/name`` (and aliases) to the handler bound
...
* :meth:`DaCLI._cmd_help` — emits the grouped help text.

Adding a command: append a :class:`SlashSpec` below, then wire the matching
handler in ``DaCLI._build_slash_handler_map`` so the registry integrity
