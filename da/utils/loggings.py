...
def _is_source_environment() -> bool:
    ...
    try:
        ...
        # Navigate up to project root (from da/utils/ to project root)
        project_root = os.path.dirname(os.path.dirname(current_dir))

        # Check for source code markers: pyproject.toml and da/ directory
        has_pyproject = os.path.exists(os.path.join(project_root, "pyproject.toml"))
        has_da_dir = os.path.exists(os.path.join(project_root, "da"))

        return has_pyproject and has_da_dir
    except Exception:
        return False
...
class DynamicLogManager:
    ...
    def __init__(self, debug=False, log_dir=None, path_manager=None, agent_config=None):
        self.debug = debug
        # Default to ~/.da/logs (via path manager) when log_dir is not specified.
        if log_dir is None:
...
def get_log_manager(
    *, path_manager: Optional["DaPathManager"] = None, agent_config: Optional[Any] = None
) -> DynamicLogManager:
...
def configure_logging(
    ...
    *,
    ...
    Args:
        ...
        log_dir: Directory for log files. If None, defaults to ``~/.da/logs``
                 (resolved via the active ``DaPathManager``).
