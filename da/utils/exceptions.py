...
from da.utils.loggings import get_log_manager, get_logger

logger = get_logger(__name__)


class ErrorCode(Enum):
    """Error codes with descriptions for DA exceptions."""
...
class DaException(Exception):
    """DA agent exception with error code."""
...
def setup_exception_handler(console_logger=None, prefix_wrap_func=None):
    """Setup global exception handler for DA
    ...
    def global_exception_handler(type, value, tb):
        ...
        log_prefix = (
            "Execution failed" if type == DaException or issubclass(type, DaException) else "Unexpected failed"
        )
