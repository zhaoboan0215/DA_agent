...
from da.models.mcp_utils import multiple_mcp_servers
...
class ClaudeModel(OpenAICompatibleModel):
    ...
    def _get_api_key(self) -> str:
        ...
        if not api_key:
            raise DaException(ErrorCode.MODEL_AUTHENTICATION_ERROR)
        return api_key
    ...
    def _diagnose_oauth_401(self, original_error: Exception) -> None:
        ...
        if expires_at is None:
            try:
                from da.auth.claude_credential import _read_keychain_credentials
        ...
        if expires_at and int(expires_at) / 1000 < time.time():
            ...
            raise DaException(ErrorCode.CLAUDE_SUBSCRIPTION_TOKEN_EXPIRED) from original_error
        ...
        raise DaException(ErrorCode.CLAUDE_SUBSCRIPTION_AUTH_FAILED) from original_error
    ...
    def generate(self, prompt: Any, enable_thinking: bool = False, **kwargs) -> str:
        ...
        try:
            ...
            self._diagnose_oauth_401(e)  # raises specific DaException for OAuth tokens
            raise
