...
class SqliteRdbDatabase(RdbDatabase):
    ...
    file under ``{data_dir}/{project}/da_db/``).
    """
    ...
    def _open_connection(self) -> sqlite3.Connection:
        ...
        try:
            ...
            raise DaException(ErrorCode.STORAGE_FAILED, message=f"Database connection error: {str(e)}") from e
...
class SqliteRdbBackend(BaseRdbBackend):
    ...
    ``.db`` file at ``{data_dir}/{project}/da_db/{store_db_name}.db``.
    ...
    def connect(self, project: str, store_db_name: str) -> SqliteRdbDatabase:
        ...
        return SqliteRdbDatabase(os.path.join(self._data_dir, safe_project, "da_db", f"{safe_store}.db"))
