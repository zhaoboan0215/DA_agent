...
"""Project-level ``./.da/config.yml`` override.
...
- ``project_name``: shard name for ``~/.da/sessions/{project_name}/``
  and ``~/.da/data/{project_name}/`` (optional)
...
PROJECT_CONFIG_REL = ".da/config.yml"
ALLOWED_KEYS = frozenset({"target", "default_datasource", "project_name", "language"})


@dataclass
class ProjectTarget:
    """Structured ``target:`` value from ``./.da/config.yml``.
...
class ProjectOverride:
    """In-memory representation of ``./.da/config.yml``.
...
def load_project_override(cwd: Optional[str] = None) -> Optional[ProjectOverride]:
    """Read ``./.da/config.yml`` relative to ``cwd``.
...
def save_project_override(override: ProjectOverride, cwd: Optional[str] = None) -> Path:
    """Write ``override`` to ``./.da/config.yml``.

    Creates the ``.da/`` parent directory if missing.  ``None`` fields
