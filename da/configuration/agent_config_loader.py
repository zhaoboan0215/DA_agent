...
from da.configuration.project_config import ProjectTarget, load_project_override
...
def parse_config_path(config_file: str = "", create_if_missing: bool = False) -> Path:
    ...
    3. ~/.da/conf/agent.yml (fixed path, not from agent.home config)

    When ``create_if_missing`` is True and no config file is found, a minimal
    agent.yml is bootstrapped at ``~/.da/conf/agent.yml`` instead of raising.
    ...
    Raises:
        DaException: If configuration file not found (and create_if_missing is False)
    """
    ...
    raise DaException(
        ...
        message=(
            "Agent configuration file not found. Please configure your `conf/agent.yaml` or `.da/conf/agent.yml`"
...
def _apply_project_override(agent_raw: Dict[str, Any]) -> None:
    """Merge ``./.da/config.yml`` overlay into the raw agent config dict.
    ...
    web, ``da-api``, SDK), not just the CLI layer.
        ...
        if isinstance(override.target, ProjectTarget):
            if override.target.custom:
                ...
                if override.target.custom not in models:
                    logger.warning(
                        "target.custom '%s' from .da/config.yml not found in agent.models; "
            ...
            if override.target not in models:
                logger.warning(
                    "target '%s' from .da/config.yml not found in agent.models; will be resolved lazily at runtime.",
                    override.target,
                )
    ...
    if override.default_datasource is not None:
        ...
        if override.default_datasource not in datasources:
            raise DaException(
                ...
                message_args={
                    "field_name": "default_datasource (from .da/config.yml)",
...
def load_agent_config(reload: bool = False, create_if_missing: bool = False, **kwargs) -> AgentConfig:
    # Check config file in order: kwargs["config"] > conf/agent.yml > ~/.da/conf/agent.yml
    ...
    #   1. ``./.da/config.yml::default_datasource`` (via _apply_project_override)
            ...
            raise DaException(
                ...
                message_args={
                    "config_error": (
                        ...
                        "./.da/config.yml is missing and agent.yml has multiple "
                        "datasources without any marked as `default: true`. Run "
                        "`DA` in this project directory first to launch the "
                        "init wizard (which writes ./.da/config.yml with your "
...
def get_agent_home(config_file: str = "") -> str:
    ...
    return raw.get("agent", {}).get("home", "~/.da")
