...
from da.schemas.base import BaseInput
from da.schemas.node_models import StrategyType
from da.storage.embedding_models import init_embedding_models
from da.storage.storage_cfg import check_storage_config, save_storage_config
from da.utils.constants import DBType
from da.utils.exceptions import DaException, ErrorCode
from da.utils.loggings import get_logger
from da.utils.path_utils import get_files_from_glob_pattern
...
# (DA.storage.rdb.sqlite_backend / DA.storage.vector.lance_backend).  Any
# character outside this class in a CWD-derived project name would cause
# backend init to raise ``DaException``, so the CWD normalizer sanitizes
...
def _validate_project_name(value: str) -> str:
    ...
    Raises:
        DaException: when ``value`` contains forbidden characters or is
...
class AgentConfig:
    ...
    def __init__(self, nodes: Dict[str, NodeConfig], **kwargs):
        ...
        if storage_config:
            if self._skip_init_dirs:
                ...
                except (DaException, KeyError, ValueError) as e:
        ...
        # Initialize channels configuration for Da Gateway IM gateway
    ...
    def _init_permissions_config(self, permissions_raw: Dict[str, Any]):
        ...
        try:
            from da.tools.permission.permission_config import PermissionConfig
    ...
    def save_run_dir(self, database: str, run_id: Optional[str] = None) -> Path:
        ...
        return DaPathManager.resolve_run_dir(Path(self._save_dir), database, run_id)
    ...
    def trajectory_run_dir(self, database: str, run_id: Optional[str] = None) -> Path:
        ...
        return DaPathManager.resolve_run_dir(Path(self._trajectory_dir), database, run_id)
    ...
    def _init_dirs(
        ...
        logger.info(f"Using da home directory: {path_manager.DA_home}")
    ...
    def override_by_args(self, **kwargs):
        ...
        if kwargs.get("benchmark", ""):
            ...
            if benchmark_platform == "spider2" and self.db_type != "snowflake":
                raise DaException(code=ErrorCode.COMMON_UNSUPPORTED, message="spider2 only support snowflake")
            if benchmark_platform == "bird_dev" and self.db_type != DBType.SQLITE:
                raise DaException(code=ErrorCode.COMMON_UNSUPPORTED, message="bird_dev only support sqlite")
    ...
    def _set_path_manager(self, home: str) -> None:
        from da.utils.path_manager import DaPathManager, set_current_path_manager

        self.path_manager = DaPathManager(
            home,
    ...
    def active_model(self) -> ModelConfig:
        ...
        1. **Provider-level** (``self._target_provider`` + ``self._target_model``
           set via ``./.da/config.yml``) — synthesize a ``ModelConfig`` by
        ...
        3. Neither configured → raise :class:`DaException`.
        ...
        raise DaException(
            ...
            message=(
                ...
                "Run the Da CLI and use the /model command to set up a provider and model:\n"
                "  1. Start CLI: da\n"
                "  2. Use /model to pick a provider and model\n"
                "Or run 'da init' for first-time setup."
            ),
        )
    ...
    def provider_catalog(self) -> Dict[str, Any]:
        ...
        ``DA.cli.provider_model_catalog`` which implements remote→cache→local
    ...
    def set_active_provider_model(self, provider: str, model: str, persist: bool = True) -> None:
        ...
        ``True`` (default), writes the selection to ``./.da/config.yml``
        ...
        if persist:
            from da.configuration.project_config import (
                ProjectOverride,
                ProjectTarget,
    ...
    def set_provider_config(
        ...
        The disk write targets the global ``~/.da/conf/agent.yml`` under
    ...
    def provider_available(self, provider: str) -> bool:
        ...
        ``DA.auth`` helpers that inspect on-disk tokens.
        """
        catalog = self.provider_catalog
        providers_meta = catalog.get("providers", {}) if isinstance(catalog, dict) else {}
        ...
        if auth_type == "subscription":
            try:
                from da.auth.claude_credential import get_claude_subscription_token
        ...
        if auth_type == "oauth":
            try:
                from da.auth.oauth_manager import OAuthManager
    ...
    def rag_storage_path(self) -> str:
        ...
        # so all isolation modes converge on the same ``da_db`` subdirectory.
        return os.path.join(self.rag_base_path, "da_db")
    ...
    def _is_file_based_vector_backend(self) -> bool:
        ...
        The ``da_db.cfg`` embedding-config check is only meaningful for
...
def _load_provider_catalog() -> Dict[str, Any]:
    ...
    try:
        ...
        from da.cli.provider_model_catalog import resolve_provider_models
        from da.utils.resource_utils import read_data_file_text
...
def _persist_provider_section(provider: str, entry: "ProviderConfig") -> None:
    ...
    """
    try:
        from da.configuration.agent_config_loader import configuration_manager
