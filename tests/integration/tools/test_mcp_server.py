...
"""
End-to-End Integration Tests for Da MCP Server
...
from da.mcp_server import DaMCPServer, create_dynamic_app, create_server
...
class TestStaticModeHTTPStreamable(StaticModeTestBase):
    """Test DaMCPServer in static mode using HTTP Streamable transport."""

    @pytest_asyncio.fixture(autouse=True)
    async def static_server(self):
        """Start a static-mode MCP server for ssb_sqlite datasource."""
        port = find_free_port()
        server = DaMCPServer(datasource="ssb_sqlite", config_path=CONFIG_PATH, stateless_http=True)
...
class TestStaticModeSSE(StaticModeTestBase):
    """Test DaMCPServer in static mode using SSE transport."""

    @pytest_asyncio.fixture(autouse=True)
    async def static_sse_server(self):
        """Start a static-mode MCP server with SSE transport for ssb_sqlite."""
        port = find_free_port()
        server = DaMCPServer(datasource="ssb_sqlite", config_path=CONFIG_PATH)
...
class TestStaticModeStdio(StaticModeTestBase):
    """Test DaMCPServer in static mode using stdio transport."""
