...
def _svc_mod():
    return sys.modules["da.api.service"]
...
@pytest.fixture(scope="module")
def chat_da_service(chat_agent_config):
    from da.api.services.da_service import DaService

    return DaService(agent_config=chat_agent_config, project_id="chat_integration_test")


@pytest_asyncio.fixture(scope="module")
async def chat_client(chat_agent_config, chat_da_service):
    """AsyncClient wired to the full FastAPI app with real DaService."""
    import da.api.deps as deps_mod
    from da.api.auth import NoAuthProvider
    from da.api.deps import init_deps
    from da.api.service import DaAPIService, create_app
    ...
    cache = DaServiceCache(max_size=4)

    async def _factory():
        return chat_da_service

    # Enter try/finally BEFORE mutating globals so failures during setup
    # (DaAPIService, init_deps, cache.get_or_create) still unwind the
    # patched module state and shut the cache down cleanly.
    try:
        mod.service = DaAPIService(agent_args)
...
async def _start_task(svc, message, session_id, *, source=None):
    ...
    from da.api.models.cli_models import StreamChatInput
...
class TestAPIChatN9:
    ...
    @pytest.mark.asyncio
    async def test_resume_from_cursor(self, chat_client, chat_da_service):
        """start task → wait for events → resume from cursor 0."""
        task = await _start_task(chat_da_service, "How many charter schools are there?", "resume_sess")
        try:
            ...
            await _cleanup_task(chat_da_service, task)
    ...
    @pytest.mark.asyncio
    async def test_stop_running_chat(self, chat_client, chat_da_service):
    ...
    @pytest.mark.asyncio
    async def test_source_proxy_tool_result(self, chat_client, chat_da_service, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_ask_user_interaction(self, chat_client, chat_da_service):
