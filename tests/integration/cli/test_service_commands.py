...
class _FakeCLI:
    """Minimal DaCLI stand-in: real agent_config + captured Rich console."""
...
class TestMissingAdapterEndToEnd:
    ...
    ``DaException`` from ``BIFuncTool._build_adapter`` at the first
    ...
    def cli_with_unregistered_platform(self, tmp_path, bi_core_stub):
        ...
        # fails the way it would on a real machine missing ``DA-bi-<x>``.
        agent_config = _build_agent_config(
            tmp_path,
