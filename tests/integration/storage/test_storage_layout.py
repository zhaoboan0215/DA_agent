...
Validates the end-to-end wiring of ``AgentConfig`` + ``DaPathManager``:

* Knowledge-base dirs land under ``{project_root}/subject/``.
* Sessions and data dirs are sharded by ``project_name`` under ``DA_home``.
* Switching ``project_name`` at runtime rebuilds the sharded paths.
* Live backends (``create_vector_connection`` / ``create_rdb_for_store``)
  actually write under ``{data_dir}/{project}/da_db`` — this catches
...
from da.configuration.agent_config import AgentConfig, NodeConfig, _normalize_project_name
...
class TestStorageLayoutIntegration:
    def test_two_projects_isolate_subject_and_data(self, tmp_path):
        ...
        DA_home = tmp_path / "home"

        proj_a_root = tmp_path / "project_a"
        proj_b_root = tmp_path / "project_b"

        cfg_a = _make_config(home=DA_home, project_name="proj_a", project_root=proj_a_root)
        cfg_b = _make_config(home=DA_home, project_name="proj_b", project_root=proj_b_root)
        ...
        assert cfg_a.path_manager.data_dir == DA_home.resolve() / "data"
        assert cfg_b.path_manager.data_dir == DA_home.resolve() / "data"
        assert cfg_a.path_manager.project_data_dir == DA_home.resolve() / "data" / "proj_a"
        assert cfg_b.path_manager.project_data_dir == DA_home.resolve() / "data" / "proj_b"
        # sessions/ is directly sharded by project_name (no backend indirection).
        assert cfg_a.path_manager.sessions_dir == DA_home.resolve() / "sessions" / "proj_a"
        assert cfg_b.path_manager.sessions_dir == DA_home.resolve() / "sessions" / "proj_b"
    ...
    def test_kb_subtree_follows_project_root(self, tmp_path):
        ...
        cfg = _make_config(home=DA_home, project_name="my_project", project_root=project_root)
        ...
        # The project-level skills directory lives under project_root/.da/skills
        assert cfg.path_manager.project_skills_dir == project_root.resolve() / ".DA" / "skills"
    ...
    def test_auto_project_name_from_cwd(self, tmp_path, monkeypatch):
        ...
        assert cfg.path_manager.data_dir == DA_home.resolve() / "data"
        assert cfg.path_manager.project_data_dir == DA_home.resolve() / "data" / expected
        assert cfg.path_manager.sessions_dir == DA_home.resolve() / "sessions" / expected
...
class TestBackendLandingPath:
    ...
    ``DaPathManager.data_dir`` and the path the backends actually open.
    """
    ...
    def test_vector_connection_lands_under_project_shard(self, tmp_path, _backends):
        ...
        try:
            expected = data_dir / "proj_x" / "da_db"
    ...
    def test_rdb_store_file_lands_under_project_shard(self, tmp_path, _backends):
        ...
        expected = data_dir / "proj_x" / "da_db" / "subject_tree.db"
