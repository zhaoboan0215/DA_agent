...
Opt-in (all required):
  * install:    `uv pip install DA-trino`
  * start:      `cd DA-db-adapters/DA-trino && docker compose up -d`
...
DA_trino = import_required(
    "DA_trino",
    reason="DA-trino not installed; run `uv pip install DA-trino`",
)

TrinoConfig = DA_trino.TrinoConfig
TrinoConnector = DA_trino.TrinoConnector
    ...
    try:
        if not conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-trino? "
