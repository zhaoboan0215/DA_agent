...
Opt-in (all required):
  * install:    `uv pip install DA-hive`
  * start:      `cd DA-db-adapters/DA-hive && docker compose up -d`
...
DA_hive = import_required(
    "DA_hive",
    reason="DA-hive not installed; run `uv pip install DA-hive`",
)

HiveConfig = DA_hive.HiveConfig
HiveConnector = DA_hive.HiveConnector
    ...
    try:
        if not conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-hive? "
