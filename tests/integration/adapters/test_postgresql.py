...
Opt-in (both required):
  * install the adapter:       `uv pip install DA-postgresql`
  * start the docker service:  `cd DA-db-adapters/DA-postgresql && docker compose up -d`
...
# DA-postgresql is NOT a hard dep of DA-agent; audit allows importorskip here
# because the package is not in [project.dependencies].
DA_postgresql = import_required(
    "DA_postgresql",
    reason="DA-postgresql not installed; run `uv pip install DA-postgresql`",
)

PostgreSQLConfig = DA_postgresql.PostgreSQLConfig
PostgreSQLConnector = DA_postgresql.PostgreSQLConnector

from da.tools.func_tool.database import DBFuncTool  # noqa: E402

pytestmark = [pytest.mark.integration, pytest.mark.nightly]


SCHEMA = os.getenv("POSTGRESQL_SCHEMA", "public")
REGION_TABLE = "DA_adapter_region"
NATION_TABLE = "DA_adapter_nation"
    ...
    try:
        if not conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-postgresql?"
            )
        yield conn
    finally:
        conn.close()
