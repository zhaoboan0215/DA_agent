...
Opt-in (all required):
  * install:    `uv pip install DA-greenplum`
  * start:      `cd DA-db-adapters/DA-greenplum && docker compose up -d`
  * env:         ADAPTERS_GP=1
...
DA_greenplum = import_required(
    "DA_greenplum",
    reason="DA-greenplum not installed; run `uv pip install DA-greenplum`",
)

GreenplumConfig = DA_greenplum.GreenplumConfig
GreenplumConnector = DA_greenplum.GreenplumConnector
    ...
    try:
        if not conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-greenplum?"
            )
        yield conn
    finally:
        conn.close()
