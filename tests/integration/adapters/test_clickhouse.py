...
Opt-in (all required):
  * install:    `uv pip install DA-clickhouse`
  * start:      `cd DA-db-adapters/DA-clickhouse && docker compose up -d`
  * env:         ADAPTERS_CH=1
...
DA_clickhouse = import_required(
    "DA_clickhouse",
    reason="DA-clickhouse not installed; run `uv pip install DA-clickhouse`",
)

ClickHouseConfig = DA_clickhouse.ClickHouseConfig
ClickHouseConnector = DA_clickhouse.ClickHouseConnector
    ...
    try:
        if not init_conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-clickhouse?"
            )
