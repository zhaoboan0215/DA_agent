...
Opt-in (all required):
  * install:    `uv pip install DA-starrocks`
  * start:      `cd DA-db-adapters/DA-starrocks && docker compose up -d`
  * env:         ADAPTERS_SR=1
...
DA_starrocks = import_required(
    "DA_starrocks",
    reason="DA-starrocks not installed; run `uv pip install DA-starrocks`",
)

StarRocksConfig = DA_starrocks.StarRocksConfig
StarRocksConnector = DA_starrocks.StarRocksConnector
    ...
    try:
        if not init_conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-starrocks?"
            )
