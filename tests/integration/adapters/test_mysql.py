...
Opt-in (all required):
  * install:    `uv pip install DA-mysql`
  * start:      `cd DA-db-adapters/DA-mysql && docker compose up -d`
...
DA_mysql = import_required(
    "DA_mysql",
    reason="DA-mysql not installed; run `uv pip install DA-mysql`",
)

MySQLConfig = DA_mysql.MySQLConfig
MySQLConnector = DA_mysql.MySQLConnector
    ...
    try:
        if not conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-mysql?"
            )
        yield conn
    finally:
        conn.close()
