...
Opt-in (all required):
  * install:    `uv pip install DA-spark`
  * start:      `cd DA-db-adapters/DA-spark && docker compose up -d`
...
DA_spark = import_required(
    "DA_spark",
    reason="DA-spark not installed; run `uv pip install DA-spark`",
)

SparkConfig = DA_spark.SparkConfig
SparkConnector = DA_spark.SparkConnector
    ...
    try:
        if not conn.test_connection():
            pytest.fail(
                ...
                "Did you run `docker compose up -d` in DA-db-adapters/DA-spark? "
