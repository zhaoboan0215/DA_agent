...
"""Unit tests for da.gateway.runtime.DaGateway."""
...
from da.gateway.models import OutboundMessage
...
def _make_gateway(channels_config: dict | None = None) -> DaGateway:
...
class TestGatewayInit:
    ...
    def test_task_manager_created_with_non_interactive_mode(self):
        """DaGateway should create ChatTaskManager with default_interactive=False."""
        agent_config = MagicMock()
        with patch("da.gateway.runtime.ChatTaskManager") as mock_ctm:
            DaGateway(agent_config=agent_config, channels_config={})
...
class TestGatewayStart:
    ...
    async def test_no_enabled_channels_returns_early(self):
        gw = _make_gateway({})
        with patch("da.gateway.runtime.register_builtins"):
            await gw.start()
    ...
    async def test_enabled_channel_instantiated_and_started(self):
        ...
        with (
            patch("da.gateway.runtime.register_builtins"),
            patch("da.gateway.runtime.get_adapter_class", return_value=_FakeAdapter),
        ):
            ...
            async def _start_with_immediate_shutdown():
                from da.gateway.models import ChannelConfig
...
class TestGatewaySignals:
    ...
    async def test_windows_platform_check_in_code(self):
        ...
        import da.gateway.runtime as gw_mod

        source = inspect.getsource(gw_mod.DaGateway.start)
