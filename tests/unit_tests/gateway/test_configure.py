...
"""Unit tests for da.gateway.configure.ChannelConfigurator."""
...
def configurator(agent_yml: Path):
    ...
    import da.configuration.agent_config_loader as loader

    loader.CONFIGURATION_MANAGER = None
    from da.gateway.configure import ChannelConfigurator
...
def test_redact_masks_secret_keys():
    from da.gateway.configure import _redact
...
def test_validate_channel_name_rules():
    from da.gateway.configure import _validate_channel_name
...
def test_add_slack_channel_preserves_existing(configurator, agent_yml):
    ...
    with (
        patch("da.gateway.configure.Prompt.ask", side_effect=lambda *a, **kw: next(prompt_responses)),
        patch("da.gateway.configure.Confirm.ask", side_effect=lambda *a, **kw: next(confirm_responses)),
        patch("da.gateway.configure.getpass", side_effect=lambda prompt="": next(getpass_responses)),
        patch("da.gateway.configure.select_choice", return_value="slack"),
    ):
        rc = configurator.add()
...
def test_add_rejects_duplicate_then_accepts(configurator, agent_yml):
    ...
    with (
        patch("da.gateway.configure.Prompt.ask", side_effect=lambda *a, **kw: next(prompts)),
        patch("da.gateway.configure.Confirm.ask", side_effect=lambda *a, **kw: next(confirms)),
        patch("da.gateway.configure.getpass", side_effect=lambda prompt="": next(passwords)),
        patch("da.gateway.configure.select_choice", return_value="feishu"),
    ):
        rc = configurator.add()
...
def test_add_triggers_pip_install_when_confirmed(configurator):
    ...
    with (
        ...
        patch("da.gateway.configure.getpass", side_effect=lambda prompt="": next(passwords)),
        patch("da.gateway.configure.select_choice", return_value="feishu"),
        patch("da.gateway.configure.subprocess.run", return_value=fake_completed) as mock_run,
    ):
        rc = configurator.add()
...
def test_delete_removes_named_channel(configurator, agent_yml):
    ...
    with patch("da.gateway.configure.Confirm.ask", return_value=True):
...
def test_delete_aborts_when_user_declines(configurator, agent_yml):
    with patch("da.gateway.configure.Confirm.ask", return_value=False):
...
def test_change_verbose_updates_value(configurator, agent_yml):
    with patch("da.gateway.configure.select_choice", return_value="detail"):
...
def test_change_verbose_noop_when_unchanged(configurator, agent_yml, capsys):
    with patch("da.gateway.configure.select_choice", return_value="brief"):
        rc = configurator._change_verbose("existing-feishu")
    assert rc == 0
...
def test_reinstall_deps_invokes_pip(configurator):
    fake_completed = MagicMock(returncode=0)
    with (
        patch("da.gateway.configure.Confirm.ask", return_value=True),
        patch("da.gateway.configure.subprocess.run", return_value=fake_completed) as mock_run,
    ):
...
def test_reinstall_deps_reports_failure(configurator):
    ...
    with (
        patch("da.gateway.configure.Confirm.ask", return_value=True),
        patch("da.gateway.configure.subprocess.run", return_value=fake_completed),
    ):
        rc = configurator._reinstall_deps("feishu")
    assert rc == 2
...
def test_channel_submenu_routes_toggle(configurator):
    with (
        patch("da.gateway.configure._select_menu", return_value="toggle_enabled"),
...
def test_channel_submenu_routes_change_verbose(configurator):
    with (
        patch("da.gateway.configure._select_menu", return_value="change_verbose"),
...
def test_channel_submenu_routes_reinstall(configurator):
    with (
        patch("da.gateway.configure._select_menu", return_value="reinstall_deps"),
...
def test_channel_submenu_routes_delete(configurator):
    with (
        patch("da.gateway.configure._select_menu", return_value="delete"),
...
def test_channel_submenu_back_is_noop(configurator):
    with (
        patch("da.gateway.configure._select_menu", return_value="back"),
        patch.object(configurator, "delete") as mock_delete,
        patch.object(configurator, "_toggle_enabled") as mock_toggle,
    ):
        configurator._channel_submenu("existing-feishu")
...
def test_channel_submenu_cancel_is_noop(configurator):
    with (
        patch("da.gateway.configure._select_menu", return_value=None),
        patch.object(configurator, "delete") as mock_delete,
    ):
        configurator._channel_submenu("existing-feishu")
    mock_delete.assert_not_called()
...
def test_render_hub_returns_add(configurator):
    with patch("da.gateway.configure._select_menu", return_value="__add__"):
        assert configurator._render_hub() == ("add", None)


def test_render_hub_returns_channel(configurator):
    with patch("da.gateway.configure._select_menu", return_value="channel:existing-feishu"):
        assert configurator._render_hub() == ("channel", "existing-feishu")


def test_render_hub_quit_on_cancel(configurator):
    with patch("da.gateway.configure._select_menu", return_value=None):
...
def test_render_hub_empty_channels(tmp_path):
    ...
    with patch("da.gateway.configure._select_menu", side_effect=_capture):
...
# ---------------------------------------------------------------------------
# CLI dispatch in DA.gateway.main
# ---------------------------------------------------------------------------
def test_main_cli_dispatches_configure(monkeypatch, agent_yml):
    from da.gateway import main as gateway_main

    fake_instance = MagicMock()
    fake_instance.run.return_value = 0
    monkeypatch.setattr("sys.argv", ["da-gateway", "configure", "--config", str(agent_yml)])

    with (
        patch("da.gateway.configure.ChannelConfigurator", return_value=fake_instance) as mock_cls,
...
def test_main_cli_parser_allows_bare_configure(monkeypatch):
    ...
    from da.gateway.main import _build_parser
...
def test_main_cli_parser_rejects_legacy_action():
    """Old `da-gateway configure list` syntax must now fail."""
    from da.gateway.main import _build_parser
