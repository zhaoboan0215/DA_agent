...
Design principle: Mock SchedulerTools since DA-scheduler-core is optional.
...
# `from da.tools.func_tool.scheduler_tools import SchedulerTools`)
_SCHEDULER_TOOLS_PATCH = "da.tools.func_tool.scheduler_tools.SchedulerTools"
...
class TestSchedulerToolSetup:
    ...
    def test_import_error_yields_no_tools(self, real_agent_config, mock_llm_create):
        ...
        _add_scheduler_config(real_agent_config)
        with patch(_SCHEDULER_TOOLS_PATCH, side_effect=ImportError("DA-scheduler-core not installed")):
            from da.agent.node.scheduler_agentic_node import SchedulerAgenticNode
...
class TestSchedulerExecuteStream:
    ...
    async def test_execute_stream_raises_without_input(self, real_agent_config, mock_llm_create):
        """execute_stream should raise DaException when self.input is None."""
        _add_scheduler_config(real_agent_config)
        with patch(_SCHEDULER_TOOLS_PATCH, return_value=_make_mock_scheduler_tools()):
            from da.agent.node.scheduler_agentic_node import SchedulerAgenticNode
...
class TestSchedulerCustomNodeName:
    ...
    def test_alias_system_prompt_template_name(self, real_agent_config, mock_llm_create):
        ...
        with patch(_SCHEDULER_TOOLS_PATCH, return_value=_make_mock_scheduler_tools()):
            ...
            with patch("da.prompts.prompt_manager.get_prompt_manager") as mock_pm:
