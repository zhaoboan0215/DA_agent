...
class TestBuildEnhancedMessage:
    ...
    def test_basic_message(self):
        from da.agent.node.gen_sql_agentic_node import build_enhanced_message
...
class TestPrepareTemplateContext:
    ...
    def test_basic_context(self):
        from da.agent.node.gen_sql_agentic_node import prepare_template_context
...
class TestSetupToolPatternGenSQL:
    ...
    def test_wildcard_unknown_type_logs_warning(self, real_agent_config, mock_llm_create):
        node = _make_node_extra2(real_agent_config, mock_llm_create)
        with patch("da.agent.node.gen_sql_agentic_node.logger.warning") as mock_warning:
    ...
    def test_exception_in_setup_is_caught(self, real_agent_config, mock_llm_create):
        ...
        with (
            ...
            patch("da.agent.node.gen_sql_agentic_node.logger.error") as mock_error,
        ):
...
def _make_gensql_node(node_config=None, agent_config=None):
    ...
    from da.agent.node.agentic_node import AgenticNode
    from da.agent.node.gen_sql_agentic_node import GenSQLAgenticNode
    from da.cli.execution_state import InteractionBroker, InterruptController
...
class TestGenSQLNodeInput:
    ...
    def test_with_schemas(self):
        from da.schemas.node_models import TableSchema
