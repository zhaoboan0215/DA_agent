...
from da.schemas.node_models import GenerateSQLInput, GenerateSQLResult, SqlTask, TableSchema, TableValue
...
class TestGenerateSQLNodeExecute:
    ...
    def test_execute_calls_generate_sql_function(self):
        ...
        with patch("da.agent.node.generate_sql_node.generate_sql", return_value=expected):
            node.execute()
    ...
    def test_execute_handles_exception(self):
        ...
        with patch(
            "da.agent.node.generate_sql_node.generate_sql",
...
class TestGenerateSQLFunction:
    ...
    def test_generate_sql_valid_json_response(self):
        from da.agent.node.generate_sql_node import generate_sql
...
class TestGenerateSQLNodeStream:
    ...
    async def test_execute_stream_yields_actions(self):
        ...
        with patch("da.agent.node.generate_sql_node.generate_sql", return_value=mock_result):
            actions = []
            async for action in node.execute_stream():
                actions.append(action)
