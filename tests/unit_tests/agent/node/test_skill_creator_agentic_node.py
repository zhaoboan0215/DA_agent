...
class TestSkillCreatorAgenticNodeTools:
    ...
    def test_has_unified_filesystem_tools(self, real_agent_config, mock_llm_create):
        ...
        enforced by GenerationHooks (``.da/skills/**``), not by a separate
        prefixed instance.
        """
        from da.agent.node.gen_skill_agentic_node import SkillCreatorAgenticNode
...
class TestSkillCreatorExecution:
    ...
    async def test_no_input_raises(self, real_agent_config, mock_llm_create):
        """execute_stream should raise DaException if no input is set."""
        from da.agent.node.gen_skill_agentic_node import SkillCreatorAgenticNode
    ...
    @pytest.mark.asyncio
    async def test_execute_stream_da_exception_handling(self, real_agent_config, mock_llm_create):
        """execute_stream should handle DaException with error code."""
        from da.agent.node.gen_skill_agentic_node import SkillCreatorAgenticNode
        ...
        async def _raise_da(*args, **kwargs):
            raise DaException(ErrorCode.COMMON_CONFIG_ERROR, message_args={"config_error": "bad config"})
            yield  # noqa: unreachable

        with patch.object(node.model, "generate_with_tools_stream", _raise_da):
            ahm = ActionHistoryManager()
            actions = []
            async for action in node.execute_stream(ahm):
                actions.append(action)
...
class TestSkillCreatorSystemPromptEdgeCases:
    ...
    def test_system_prompt_template_error(self, real_agent_config, mock_llm_create):
        """System prompt should raise DaException on template error."""
        from da.agent.node.gen_skill_agentic_node import SkillCreatorAgenticNode
        from da.utils.exceptions import DaException
        ...
        with patch("da.prompts.prompt_manager.get_prompt_manager") as mock_gpm:
