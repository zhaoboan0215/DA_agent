...
from da.agent.node.schema_linking_node import SchemaLinkingNode
from da.schemas.action_history import ActionStatus
from da.schemas.node_models import TableSchema, TableValue
...
class TestExecuteSchemaLinking:
    ...
    def test_execute_uses_tool_when_rag_path_exists(self, tmp_path):
        ...
        with patch("da.agent.node.schema_linking_node.SchemaLineageTool") as mock_tool_class:
...
class TestExecuteSchemaLinkingFallback:
    def test_fallback_calls_db_manager(self):
        ...
        with patch(
            "da.tools.db_tools.db_manager.db_manager_instance",
...
class TestSearchExternalKnowledge:
    def test_returns_empty_when_knowledge_size_zero(self):
        node = _make_node()
        with patch("da.agent.node.schema_linking_node.ExtKnowledgeRAG") as mock_rag_class:
    ...
    def test_returns_empty_on_exception(self):
        node = _make_node()
        with patch("da.agent.node.schema_linking_node.ExtKnowledgeRAG", side_effect=RuntimeError("rag error")):
            result = node._search_external_knowledge("test query")
