...
class TestSubworkflowNodeExecute:
    ...
    def test_execute_success(self):
        ...
        with patch("da.agent.node.subworkflow_node.generate_workflow", return_value=mock_subworkflow):
            node.execute()
    ...
    def test_execute_with_start_node_and_one_child(self):
        ...
        with patch("da.agent.node.subworkflow_node.generate_workflow", return_value=mock_subworkflow):
            # patch setup_node_input at DA.agent.evaluate since it's lazily imported
            with patch("da.agent.evaluate.setup_node_input", return_value={"success": True}):
                node.execute()
    ...
    def test_execute_setup_node_input_fails(self):
        ...
        with patch("da.agent.node.subworkflow_node.generate_workflow", return_value=mock_subworkflow):
            with patch(
                "da.agent.evaluate.setup_node_input",
