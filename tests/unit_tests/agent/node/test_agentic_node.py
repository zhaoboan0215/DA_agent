...
class TestGetOrCreateSession:
    ...
    def test_ephemeral_creates_in_memory_session(self):
        ...
        with patch("da.agent.node.agentic_node.AdvancedSQLiteSession") as mock_sqlite_cls:
...
class TestResolveLanguageName:
    def test_known_codes_return_human_names(self):
        from da.agent.node.agentic_node import _resolve_language_name
...
class TestInjectResponseLanguage:
    ...
    def test_render_failure_returns_base_unchanged(self):
        node = _make_node(agent_config=self._agent_config("zh"))
        with patch("da.agent.node.agentic_node.get_prompt_manager") as mgr:
