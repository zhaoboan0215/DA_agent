...
from da.schemas.node_models import ReflectionInput, ReflectionResult, SQLContext, SqlTask
...
class TestReflectNodeExecute:
    ...
    def test_execute_calls_evaluate_with_model(self):
        ...
        with patch("da.agent.node.reflect_node.evaluate_with_model", return_value=eval_return):
            node.execute()
...
class TestReflectNodeExecuteStrategy:
    ...
    def test_strategy_max_round_triggers_reasoning(self):
        ...
        with patch("da.agent.node.reflect_node.get_env_int", return_value=3):
