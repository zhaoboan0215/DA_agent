...
from da.schemas.node_models import ExecuteSQLResult, GenerateSQLResult
from da.schemas.schema_linking_node_models import SchemaLinkingResult
...
class TestNodeInstantiation:
    ...
    def test_node_invalid_type_in_constructor(self):
        ...
        from da.agent.node.generate_sql_node import GenerateSQLNode
...
class TestNodeInitialize:
    ...
    def test_initialize_with_model_already_set_as_llm(self):
        ...
        with patch("da.agent.node.node.LLMBaseModel.create_model") as mock_create:
            node._initialize()
    ...
    def test_initialize_with_model_name_string(self):
        ...
        with patch("da.agent.node.node.LLMBaseModel.create_model", return_value=mock_llm) as mock_create:
            node._initialize()
    ...
    def test_initialize_from_agent_config_nodes(self):
        ...
        with patch("da.agent.node.node.LLMBaseModel.create_model", return_value=mock_llm):
            node._initialize()
            assert node.model == mock_llm
...
class TestNodeFromDict:
    ...
    @patch("da.agent.node.gen_table_agentic_node.DBFuncTool")
    @patch("da.models.base.LLMBaseModel.create_model")
