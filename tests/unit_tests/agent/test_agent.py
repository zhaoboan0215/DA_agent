...
"""
Unit tests for da/agent/agent.py — bootstrap_platform_doc and _print_platform_doc_result.
...
from da.agent.agent import Agent, _print_platform_doc_result, bootstrap_platform_doc
from da.schemas.action_history import ActionHistory, ActionRole, ActionStatus
from da.schemas.batch_events import BatchEvent, BatchStage
...
def _make_agent(args=None, config=None):
    ...
    with patch("da.agent.agent.db_manager_instance", return_value=mock_db_manager):
        agent = Agent(
...
class TestAgentInit:
    ...
    def test_db_manager_from_argument(self):
        ...
        args = _make_args()
        cfg = _make_agent_config()
        with patch("da.agent.agent.db_manager_instance"):
    ...
    def test_db_manager_created_when_not_provided(self):
        ...
        cfg = _make_agent_config()
        mock_db = MagicMock()
        with patch("da.agent.agent.db_manager_instance", return_value=mock_db):
...
class TestProbeLlm:
    def test_success(self):
        ...
        with patch("da.agent.agent.LLMBaseModel.create_model", return_value=mock_model):
    ...
    def test_failure_returns_error(self):
        ...
        with patch("da.agent.agent.LLMBaseModel.create_model", side_effect=RuntimeError("no key")):
            result = agent.probe_llm()
...
class TestBootstrapKbMetadata:
    ...
    def test_metadata_check_strategy_dir_exists(self, tmp_path):
        ...
        with patch("da.agent.agent.SchemaWithValueRAG", return_value=mock_store):
    ...
    def test_metadata_overwrite_local(self, tmp_path):
        ...
        with (
            patch("da.agent.agent.SchemaWithValueRAG", return_value=mock_store),
            patch("da.agent.agent.init_local_schema"),
    ...
    def test_metadata_spider2_benchmark(self):
        ...
        with (
            patch("da.agent.agent.SchemaWithValueRAG", return_value=mock_store),
            patch("da.agent.agent.init_snowflake_schema"),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_metadata_bird_dev_benchmark(self):
        ...
        with (
            patch("da.agent.agent.SchemaWithValueRAG", return_value=mock_store),
            patch("da.agent.agent.init_dev_schema"),
            patch.object(agent, "check_db", return_value={"status": "success"}),
        ):
            result = agent.bootstrap_kb()
...
class TestBootstrapKbSemanticModel:
    def test_semantic_model_overwrite_success(self, tmp_path):
        ...
        with (
            patch("da.agent.agent.SemanticModelRAG", return_value=mock_rag),
            patch("da.agent.agent.init_success_story_semantic_model", return_value=(True, None)),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_semantic_model_failure(self):
        ...
        with (
            patch("da.agent.agent.SemanticModelRAG", return_value=mock_rag),
            patch("da.agent.agent.init_success_story_semantic_model", return_value=(False, "error msg")),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_semantic_model_overwrite_cancelled_when_dir_exists(self, tmp_path):
        ...
        with (
            patch("da.agent.agent.SemanticModelRAG", return_value=mock_rag),
            patch("da.agent.agent.safe_rmtree", return_value=False),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_semantic_model_with_semantic_yaml(self):
        ...
        with (
            patch("da.agent.agent.SemanticModelRAG", return_value=mock_rag),
            patch("da.agent.agent.init_semantic_yaml_semantic_model", return_value=(True, None)),
        ):
            result = agent.bootstrap_kb()
...
class TestBootstrapKbMetrics:
    def test_metrics_overwrite_success(self):
        ...
        with (
            patch("da.agent.agent.MetricRAG", return_value=mock_rag),
            patch("da.agent.agent.init_success_story_metrics", return_value=(True, None, {})),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_metrics_with_semantic_yaml(self):
        ...
        with (
            patch("da.agent.agent.MetricRAG", return_value=mock_rag),
            patch("da.agent.agent.init_semantic_yaml_metrics", return_value=(True, None)),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_metrics_failure(self):
        ...
        with (
            patch("da.agent.agent.MetricRAG", return_value=mock_rag),
            patch("da.agent.agent.init_success_story_metrics", return_value=(False, "fail msg", {})),
        ):
            result = agent.bootstrap_kb()
...
class TestBootstrapKbExtKnowledge:
    def test_ext_knowledge_overwrite_with_csv(self):
        ...
        with (
            patch("da.agent.agent.ExtKnowledgeRAG", return_value=mock_rag),
            patch("da.agent.agent.init_ext_knowledge"),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_ext_knowledge_with_success_story(self):
        ...
        with (
            patch("da.agent.agent.ExtKnowledgeRAG", return_value=mock_rag),
            patch("da.agent.agent.init_success_story_knowledge", return_value=(True, None)),
        ):
            result = agent.bootstrap_kb()
    ...
    def test_ext_knowledge_success_story_failure(self):
        ...
        with (
            patch("da.agent.agent.ExtKnowledgeRAG", return_value=mock_rag),
            patch("da.agent.agent.init_success_story_knowledge", return_value=(False, "gen failed")),
        ):
            result = agent.bootstrap_kb()
...
class TestBootstrapKbReferenceSql:
    def test_reference_sql_overwrite_success(self):
        ...
        with (
            patch("da.storage.reference_sql.ReferenceSqlRAG", MagicMock(return_value=mock_rag), create=True),
            patch(
                "da.storage.reference_sql.reference_sql_init.init_reference_sql",
                return_value=mock_init_result,
                create=True,
            ),
        ):
            # Patch at the agent module level where it's imported
            import da.agent.agent as agent_module
...
class TestEvaluation:
    ...
    def test_evaluation_delegates_to_benchmark_utils(self):
        ...
        with patch("da.utils.benchmark_utils.evaluate_benchmark_and_report", return_value=mock_eval_result):
            result = agent.evaluation()
