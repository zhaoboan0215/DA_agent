...
class TestNoAdapterInstalled:
    ...
    def test_prompt_options_raises_when_no_adapters(self, empty_registry_commands):
        ...
        with pytest.raises(ValueError, match="No BI adapter implementations found.*pip install da-agent"):
    ...
    def test_create_adapter_raises_for_unknown_platform(self, empty_registry_commands):
        ...
        from da.cli.bi_dashboard import DashboardCliOptions
        ...
        with pytest.raises(ValueError, match="Unsupported platform 'superset'.*pip install DA-bi-superset"):
