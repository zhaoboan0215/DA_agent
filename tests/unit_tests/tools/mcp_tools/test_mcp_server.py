...
class TestMCPServerCreation:
    ...
    @pytest.fixture
    def server(self):
        with patch.object(DaMCPServer, "_init_tools"), patch.object(DaMCPServer, "_register_tools"):
...
class TestDynamicModeHTTPEndpoints:
    ...
    async def test_root_endpoint(self, http_app):
        ...
        async with httpx.AsyncClient(
            ...
            data = response.json()
            assert data["service"] == "Da MCP Server"
