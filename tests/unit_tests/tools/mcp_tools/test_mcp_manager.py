...
"""Unit tests for da/tools/mcp_tools/mcp_manager.py"""
...
from da.tools.mcp_tools.mcp_manager import MCPManager, _validate_server_exists, create_static_tool_filter
...
def _make_manager(tmp_path: Path) -> MCPManager:
    ...
    with patch("da.utils.path_manager.get_path_manager", return_value=mock_path_manager):
        manager = MCPManager()
...
class TestCreateServerInstance:
    def test_create_stdio_server(self, tmp_path):
        ...
        with patch("da.tools.mcp_tools.mcp_manager.SilentMCPServerStdio") as mock_cls:
    ...
    def test_create_sse_server_with_url(self, tmp_path):
        manager = _make_manager(tmp_path)
        with patch("da.tools.mcp_tools.mcp_manager.MCPServerSse") as mock_cls:
            mock_instance = MagicMock()
            mock_cls.return_value = mock_instance
            with patch("da.tools.mcp_tools.mcp_manager.MCPServerSseParams"):
                instance, details = manager._create_sse_server({"url": "http://example.com"})
        assert instance is not None

    def test_create_http_server_with_url(self, tmp_path):
        manager = _make_manager(tmp_path)
        with patch("da.tools.mcp_tools.mcp_manager.MCPServerStreamableHttp") as mock_cls:
            mock_instance = MagicMock()
            mock_cls.return_value = mock_instance
            with patch("da.tools.mcp_tools.mcp_manager.MCPServerStreamableHttpParams"):
