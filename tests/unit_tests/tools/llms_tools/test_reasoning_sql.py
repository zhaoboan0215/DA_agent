...
"""Unit tests for da/tools/llms_tools/reasoning_sql.py"""
...
from da.schemas.node_models import SqlTask, TableSchema
from da.schemas.reason_sql_node_models import ReasoningInput, ReasoningResult
from da.tools.llms_tools.reasoning_sql import reasoning_sql_with_mcp, reasoning_sql_with_mcp_stream
...
class TestReasoningSqlWithMcpStream:
    ...
    def test_yields_actions_from_base_stream(self):
        ...
        with (
            patch("da.tools.llms_tools.reasoning_sql.get_reasoning_prompt", return_value="prompt"),
            patch("da.tools.llms_tools.reasoning_sql.base_mcp_stream", return_value=fake_base_stream()),
        ):
            results = asyncio.run(
                _collect_stream(
                    reasoning_sql_with_mcp_stream(
    ...
    def test_creates_action_history_manager_when_none(self):
        ...
        with (
            patch("da.tools.llms_tools.reasoning_sql.get_reasoning_prompt", return_value="p"),
            patch("da.tools.llms_tools.reasoning_sql.base_mcp_stream", return_value=fake_base_stream()),
            patch(
                "da.tools.llms_tools.reasoning_sql.ActionHistoryManager", wraps=ActionHistoryManager
            ) as mock_ahm_cls,
        ):
            asyncio.run(
                _collect_stream(
    ...
    def test_sql_contexts_attribute_initialized_on_manager(self):
        ...
        with (
            patch("da.tools.llms_tools.reasoning_sql.get_reasoning_prompt", return_value="p"),
            patch(
                "da.tools.llms_tools.reasoning_sql.base_mcp_stream",
...
class TestReasoningSqlWithMcp:
    ...
    def test_returns_reasoning_result_from_json_content(self):
        ...
        with (
            patch("da.tools.llms_tools.reasoning_sql.get_reasoning_prompt", return_value="p"),
            patch("da.tools.llms_tools.reasoning_sql.get_prompt_manager") as mock_gpm,
            patch("da.tools.llms_tools.reasoning_sql.asyncio") as mock_asyncio,
            patch(
                "da.tools.llms_tools.reasoning_sql.llm_result2json",
    ...
    def test_falls_back_to_sql_extraction_when_json_fails(self):
        ...
        with (
            ...
            patch("da.tools.llms_tools.reasoning_sql.get_prompt_manager") as mock_gpm,
            patch("da.tools.llms_tools.reasoning_sql.asyncio") as mock_asyncio,
            patch("da.tools.llms_tools.reasoning_sql.llm_result2json", return_value=None),
            patch("da.tools.llms_tools.reasoning_sql.llm_result2sql", return_value="SELECT id FROM users"),
        ):
            mock_pm = mock_gpm.return_value
            mock_pm.get_raw_template.return_value = "instruction"
            mock_asyncio.run.return_value = exec_result
    ...
    def test_raises_da_exception_when_both_json_and_sql_fail(self):
        mock_model = MagicMock()
        input_data = _make_reasoning_input()
        ...
        with (
            ...
            patch("da.tools.llms_tools.reasoning_sql.asyncio") as mock_asyncio,
            patch("da.tools.llms_tools.reasoning_sql.llm_result2json", return_value=None),
            patch("da.tools.llms_tools.reasoning_sql.llm_result2sql", return_value=None),
        ):
            mock_pm = mock_gpm.return_value
            mock_pm.get_raw_template.return_value = "instruction"
            mock_asyncio.run.return_value = exec_result
    ...
    def test_generic_exception_raises_da_exception(self):
        mock_model = MagicMock()
        input_data = _make_reasoning_input()
    ...
    def test_uses_max_turns_from_tool_config(self):
        ...
        with (
            ...
            patch("da.tools.llms_tools.reasoning_sql.get_prompt_manager") as mock_gpm,
            patch("da.tools.llms_tools.reasoning_sql.asyncio") as mock_asyncio,
            patch("da.tools.llms_tools.reasoning_sql.llm_result2json", return_value={"sql": "SELECT 1"}),
        ):
            mock_pm = mock_gpm.return_value
            mock_pm.get_raw_template.return_value = "instruction"
