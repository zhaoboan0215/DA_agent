...
"""Unit tests for da/tools/func_tool/filesystem_tools.py"""

from pathlib import Path

from da.tools.func_tool.filesystem_tools import FilesystemConfig, FilesystemFuncTool
from da.tools.func_tool.fs_path_policy import PathZone
...
# ---------------------------------------------------------------------------
# Zone-based visibility contract: .da/ invisible except skills/memory
...
class TestPathZones:
    ...
    def test_hidden_dot_da_invisible_on_read(self, tmp_path):
        secret = tmp_path / ".DA" / "sessions" / "x.db"
        secret.parent.mkdir(parents=True)
        secret.write_text("secret")
        tool = _make_tool(str(tmp_path))
        resolved = tool._classify(".da/sessions/x.db")
        assert resolved.zone == PathZone.HIDDEN
        result = tool.read_file(".da/sessions/x.db")
        # Same "File not found" whether the file exists or not.
        assert result.success == 0
        assert "file not found" in (result.error or "").lower()

    def test_hidden_dot_da_invisible_on_write(self, tmp_path):
        tool = _make_tool(str(tmp_path))
        result = tool.write_file(".da/sessions/new.md", "payload")
        assert result.success == 0
        assert "file not found" in (result.error or "").lower()
        assert not (tmp_path / ".DA" / "sessions" / "new.md").exists()

    def test_whitelist_project_skills_readable_and_writable(self, tmp_path):
        tool = _make_tool(str(tmp_path))
        resolved = tool._classify(".da/skills/my_skill/SKILL.md")
        assert resolved.zone == PathZone.WHITELIST
        assert tool.write_file(".da/skills/my_skill/SKILL.md", "# skill\n").success == 1
        assert tool.read_file(".da/skills/my_skill/SKILL.md").result == "# skill\n"

    def test_whitelist_per_node_memory_isolated(self, tmp_path):
        tool = _make_tool(str(tmp_path), current_node="gen_sql")
        own_zone = tool._classify(".da/memory/gen_sql/MEMORY.md").zone
        other_zone = tool._classify(".da/memory/chat/MEMORY.md").zone
    ...
    def test_current_node_none_degrades_memory_to_hidden(self, tmp_path):
        ...
        zone = tool._classify(".da/memory/anything/MEMORY.md").zone
    ...
    def test_glob_prunes_hidden_subtree(self, tmp_path):
        ...
        skill_dir = tmp_path / ".DA" / "skills" / "foo"
        skill_dir.mkdir(parents=True)
        (skill_dir / "SKILL.md").write_text("skill")
        hidden = tmp_path / ".DA" / "sessions"
    ...
    def test_strict_allows_internal_and_whitelist(self, tmp_path):
        ...
        tool = FilesystemFuncTool(root_path=str(project), current_node="chat", strict=True)
        assert tool.read_file("hello.md").result == "hi"
        assert tool.write_file(".da/skills/x/SKILL.md", "# skill\n").success == 1
