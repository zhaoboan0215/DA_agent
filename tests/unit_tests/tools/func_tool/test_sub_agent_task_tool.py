...
from da.schemas.action_history import SUBAGENT_COMPLETE_ACTION_TYPE, ActionHistory, ActionRole, ActionStatus
from da.tools.func_tool.sub_agent_task_tool import (
...
class TestGetAvailableTypes:
    ...
    def test_explicit_list_filters_out_unknown_types(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="da.tools.func_tool.sub_agent_task_tool"):
            types = tool._get_available_types()
...
class TestNodeCreation:
    def test_always_creates_fresh_node(self, task_tool):
        ...
        with patch("da.agent.node.node.Node.new_instance", side_effect=[node_a, node_b]):
...
class TestActionBusIntegration:
    ...
    async def test_actions_forwarded_to_bus(self, task_tool):
        ...
        from da.schemas.action_bus import ActionBus
        from da.schemas.action_history import ActionRole
...
class TestInteractionBrokerPassthrough:
    def test_set_interaction_broker(self, task_tool):
        ...
        from da.cli.execution_state import InteractionBroker
...
class TestCreateBuiltinNode:
    ...
    @patch("da.agent.node.gen_job_agentic_node.GenJobAgenticNode.__init__", return_value=None)
    ...
    def test_create_node_custom_passes_is_subagent_true(self, task_tool):
        ...
        """
        with patch("da.agent.node.node.Node.new_instance", return_value=Mock()) as mock_new_instance:
...
class TestBuiltinNodeInheritsExecutionMode:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "gen_semantic_model",
                "da.agent.node.gen_semantic_model_agentic_node.GenSemanticModelAgenticNode.__init__",
            ),
            ("gen_metrics", "da.agent.node.gen_metrics_agentic_node.GenMetricsAgenticNode.__init__"),
            ("gen_sql_summary", "da.agent.node.sql_summary_agentic_node.SqlSummaryAgenticNode.__init__"),
            (
                "gen_ext_knowledge",
                "da.agent.node.gen_ext_knowledge_agentic_node.GenExtKnowledgeAgenticNode.__init__",
            ),
            ("gen_table", "da.agent.node.gen_table_agentic_node.GenTableAgenticNode.__init__"),
            ("gen_dashboard", "da.agent.node.gen_dashboard_agentic_node.GenDashboardAgenticNode.__init__"),
            ("scheduler", "da.agent.node.scheduler_agentic_node.SchedulerAgenticNode.__init__"),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "subagent_type,init_path",
        [
            ("gen_sql", "da.agent.node.gen_sql_agentic_node.GenSQLAgenticNode.__init__"),
            ("gen_report", "da.agent.node.gen_report_agentic_node.GenReportAgenticNode.__init__"),
            ("gen_skill", "da.agent.node.gen_skill_agentic_node.SkillCreatorAgenticNode.__init__"),
        ],
    )
...
class TestBuildNodeInputBuiltIn:
    ...
    def test_gen_table_node_input(self, task_tool):
        from da.agent.node.gen_table_agentic_node import GenTableAgenticNode
        from da.schemas.semantic_agentic_node_models import SemanticNodeInput
...
class TestProxyToolPropagation:
    ...
    async def test_apply_proxy_tools_called_when_parent_has_patterns(self, task_tool):
        ...
        with patch.object(task_tool, "_create_node", return_value=mock_node):
            with patch.object(task_tool, "_build_node_input", return_value=Mock()):
                with patch("da.tools.proxy.proxy_tool.apply_proxy_tools") as mock_apply:
                    result = await task_tool.task(type="gen_sql", prompt="test")
