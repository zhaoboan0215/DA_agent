...
class TestUnknownDatasourceRaises:
    """When _get_connector raises DaException, all three wrappers propagate it."""
    ...
    def test_get_migration_capabilities_propagates_DA_exception(self, mixin_connector):
    ...
    def test_suggest_table_layout_propagates_DA_exception(self, mixin_connector):
        tool = self._tool_with_raising_get_connector(mixin_connector)
        result = tool.suggest_table_layout(datasource="nonexistent", columns_json="[]")
        assert result.success == 0
        assert "not configured" in result.error

    def test_validate_ddl_propagates_DA_exception(self, mixin_connector):
        tool = self._tool_with_raising_get_connector(mixin_connector)
