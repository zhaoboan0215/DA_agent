...
def generation_tools(mock_agent_config):
    with (
        patch("da.tools.func_tool.generation_tools.MetricRAG") as mock_metric_rag_cls,
        patch("da.tools.func_tool.generation_tools.SemanticModelRAG") as mock_semantic_rag_cls,
    ):
...
class TestAvailableTools:
    def test_returns_four_tools(self, generation_tools):
        with patch("da.tools.func_tool.generation_tools.trans_to_function_tool") as mock_trans:
...
class TestCheckSemanticObjectExists:
    def test_table_found(self, generation_tools):
        ...
        with patch("da.tools.func_tool.generation_tools.And"), patch("da.tools.func_tool.generation_tools.eq"):
    ...
    def test_metric_found(self, generation_tools):
        ...
        with patch("da.tools.func_tool.generation_tools.eq"):
...
class TestEndSemanticModelGeneration:
    ...
    def test_exception_returns_failure(self, generation_tools):
        ...
        with patch("da.tools.func_tool.generation_tools.logger") as mock_logger:
...
class TestEndMetricGeneration:
    def _patch_sync(self, generation_tools):
        ...
        return (
            patch("da.tools.func_tool.generation_tools.get_path_manager"),
            patch.object(
...
class TestEndMetricGenerationPreflight:
    ...
    def _patch_path_resolution(tools, kb_root):
        ...
        return patch(
            "da.tools.func_tool.generation_tools.get_path_manager",
...
class TestSyncMetricToDb:
    ...
    def test_metric_only_sync(self, generation_tools, tmp_path):
        ...
        with patch("da.cli.generation_hooks.GenerationHooks._sync_semantic_to_db") as mock_sync:
...
class TestGenerateSqlSummaryId:
    def test_success(self, generation_tools):
        with patch("da.storage.reference_sql.init_utils.gen_reference_sql_id", return_value="abc123"):
