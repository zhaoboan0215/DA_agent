...
"""Unit tests for da.storage.document.doc_init."""
...
from da.storage.document.doc_init import (
    _VERSION_PATH_RE,
    InitResult,
    VersionStats,
...
class TestInitPlatformDocsDbPathDeprecation:
    ...
    def test_db_path_emits_deprecation_warning(self):
        ...
        with (
            ...
            patch("da.storage.document.doc_init.document_store") as mock_store_fn,
        ):
...
class TestInitPlatformDocsEmit:
    ...
    @patch("da.storage.document.doc_init.document_store")
    ...
    def test_emit_task_failed_on_store_error(self, mock_store_fn):
        ...
        mock_store_fn.side_effect = DaException(ErrorCode.COMMON_VALIDATION_FAILED)
    ...
    def test_emit_task_failed_on_processing_exception(self, mock_store_fn):
        ...
        with inner_patch("da.storage.document.doc_init.LocalFetcher") as mock_fetcher_cls:
...
class TestInitPlatformDocsOverwrite:
    ...
    @patch("da.storage.document.doc_init.StreamingDocProcessor")
    @patch("da.storage.document.doc_init.LocalFetcher")
    @patch("da.storage.document.doc_init.document_store")
    def test_overwrite_local_source_processes_documents(self, mock_store_fn, mock_fetcher_cls, mock_processor_cls):
        ...
        from da.storage.document.streaming_processor import ProcessingStats
    ...
    @patch("da.storage.document.doc_init.StreamingDocProcessor")
    @patch("da.storage.document.doc_init.GitHubFetcher")
    @patch("da.storage.document.doc_init.document_store")
    ...
    @patch("da.storage.document.doc_init.StreamingDocProcessor")
    @patch("da.storage.document.doc_init.WebFetcher")
    @patch("da.storage.document.doc_init.document_store")
