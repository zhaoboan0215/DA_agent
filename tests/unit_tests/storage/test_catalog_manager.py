...
"""Tests for da/storage/catalog_manager.py — CatalogUpdater pure logic methods."""

import json

from da.storage.catalog_manager import CatalogUpdater
...
class TestUpdateColumnsMethod:
    ...
    def test_update_columns_value_error_is_handled(self):
        """DaException from update_entry (entry not found) is caught and logged, not raised."""
        updater = self._make_updater()
        calls = []
...
class TestUpdateSemanticModel:
    ...
    def test_update_semantic_model_description_value_error_handled(self):
        """DaException from update_entry for table is caught and does not raise."""
        updater = self._make_updater()
        calls = []
    ...
    def test_update_semantic_model_description_propagates_non_not_found_exception(self):
        """Non-NOT_FOUND DaException from update_entry on table must propagate."""
        import pytest
    ...
    def test_update_semantic_model_columns_propagates_non_not_found_exception(self):
        """Non-NOT_FOUND DaException from update_entry on a column must propagate."""
        import pytest
