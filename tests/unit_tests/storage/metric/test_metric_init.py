...
"""Unit tests for da.storage.metric.metric_init."""
...
from da.storage.metric.metric_init import BIZ_NAME, _action_status_value, init_semantic_yaml_metrics
...
class TestInitSemanticYamlMetrics:
    ...
    def test_existing_file_calls_process(self, tmp_path):
        ...
        with patch(
            "da.storage.semantic_model.semantic_model_init.process_semantic_yaml_file",
            return_value=(True, ""),
        ) as mock_process:
...
class TestInitSuccessStoryMetricsAsync:
    ...
    def test_async_function_is_importable(self):
        ...
        from da.storage.metric.metric_init import init_success_story_metrics_async
    ...
    async def test_batch_flow_pins_prompt_version_1_1(self):
        ...
        with (
            patch("da.storage.metric.metric_init.extract_tables_from_sql_list", return_value=[]),
            patch("da.storage.metric.metric_init.GenMetricsAgenticNode", return_value=mock_node),
            patch("da.storage.metric.metric_init.pd.read_csv") as mock_read_csv,
        ):
