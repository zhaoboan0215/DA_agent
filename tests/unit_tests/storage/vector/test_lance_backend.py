...
from da.storage.vector.lance_backend import LanceVectorBackend, LanceVectorDatabase, LanceVectorTable
...
class TestLanceVectorDatabase:
    ...
    def test_create_table_with_embedding_function(self):
        ...
        with (
            unittest.mock.patch("DA.storage.vector.lance_backend._wrap_embedding") as mock_wrap,
            unittest.mock.patch("DA.storage.vector.lance_backend.EmbeddingFunctionConfig") as mock_config_cls,
        ):
...
class TestLanceVectorBackendConnect:
    ...
    def test_connect_builds_project_scoped_dir(self):
        """Each project gets ``{data_dir}/{project}/da_db``."""
        backend = LanceVectorBackend()
        backend.initialize({"data_dir": "/tmp/test"})
        with unittest.mock.patch("DA.storage.vector.lance_backend.lancedb") as mock_lancedb:
            mock_lancedb.connect.return_value = MagicMock()
            backend.connect("proj_a")
            mock_lancedb.connect.assert_called_once_with("/tmp/test/proj_a/da_db")

    def test_connect_empty_project_raises(self):
        """connect('') raises DaException instead of silently un-sharding."""
        backend = LanceVectorBackend()
        backend.initialize({"data_dir": "/tmp/test"})
    ...
    def test_single_instance_reused_across_projects(self):
        ...
        with unittest.mock.patch("DA.storage.vector.lance_backend.lancedb") as mock_lancedb:
            ...
            assert "/tmp/test/proj_a/da_db" in paths
            assert "/tmp/test/proj_b/da_db" in paths
...
class TestWrapEmbedding:
    ...
    def test_wrap_fastembed(self):
        ...
        from da.storage.vector.lance_backend import _LanceFastEmbedAdapter, _wrap_embedding
    ...
    def test_wrap_openai(self):
        ...
        from da.storage.vector.lance_backend import _LanceOpenAIAdapter, _wrap_embedding
    ...
    def test_wrap_passthrough_lance_embedding(self):
        ...
        from da.storage.vector.lance_backend import _wrap_embedding
