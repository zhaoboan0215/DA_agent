...
Test environments are discovered via entry points:
    DA.storage.rdb.testing    -- RDB test environment providers
    DA.storage.vector.testing -- Vector test environment providers
...
def _discover_via_entry_points() -> List[BackendTestConfig]:
    ...
    Discovery flow:
        1. Scan entry_points("DA.storage.rdb.testing")
        2. For each: factory = ep.load(), env = factory(), env.setup()
        3. Scan entry_points("DA.storage.vector.testing")
    ...
    rdb_factories = _load_entry_points("DA.storage.rdb.testing")
    ...
    vector_factories = _load_entry_points("DA.storage.vector.testing")
