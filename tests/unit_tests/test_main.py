...
from da.main import create_parser, main
...
def _run_main(argv):
    ...
    with patch.object(sys, "argv", ["da"] + argv):
        return main()
...
class TestMainNoAction:
    def test_no_action_prints_help_and_returns_1(self):
        ...
        with patch.object(sys, "argv", ["da"]):
            with patch("da.main.create_parser", return_value=parser):
...
class TestMainInitAction:
    def test_init_action_runs_init_workspace(self):
        ...
        with (
            patch("da.main.configure_logging"),
            patch("da.cli.init_workspace.InitWorkspace", return_value=mock_init),
            patch.object(sys, "argv", ["da", "init"]),
        ):
            result = main()
...
class TestMainTutorialAction:
    def test_tutorial_action_runs_tutorial(self):
        ...
        with (
            patch("da.main.configure_logging"),
            patch("da.main.BenchmarkTutorial", return_value=mock_tutorial),
            patch.object(sys, "argv", ["da", "tutorial"]),
        ):
            result = main()
...
class TestMainServiceAction:
    def test_service_list_runs_service_manager(self):
        ...
        with (
            patch("da.main.configure_logging"),
            patch("da.cli.service_manager.ServiceManager", return_value=mock_mgr),
            patch.object(sys, "argv", ["da", "service", "list"]),
        ):
            main()
...
class TestMainSkillAction:
    def test_skill_action_calls_run_skill_command(self):
        ...
        with (
            patch("da.main.configure_logging"),
            patch("da.main.run_skill_command", mock_run, create=True),
            patch.object(sys, "argv", ["da", "skill", "list"]),
        ):
            # Patch the import inside main
            with patch.dict("sys.modules", {"da.cli.skill_cli": MagicMock(run_skill_command=mock_run)}):
                result = main()
        assert result == 0
...
class TestMainCheckDbAction:
    def test_check_db_action(self):
        ...
        with (
            patch("da.main.configure_logging"),
            patch("da.main.setup_exception_handler"),
            patch("da.main.load_agent_config", return_value=mock_config),
            patch("da.main.Agent", return_value=mock_agent),
            patch.object(sys, "argv", ["da", "check-db", "--datasource", "myns"]),
        ):
            result = main()
...
class TestMainProbeLlmAction:
    def test_probe_llm_action(self):
        ...
        with (
            ...
            patch("da.main.load_agent_config", return_value=mock_config),
            patch("da.main.Agent", return_value=mock_agent),
            patch.object(sys, "argv", ["da", "probe-llm"]),
        ):
            result = main()
    ...
    def test_run_action_creates_sql_task(self):
        ...
        with (
            ...
            patch.object(
                sys,
                "argv",
                ["da", "run", "--datasource", "ns", "--task", "count rows", "--task_db_name", "mydb"],
            ),
        ):
            result = main()
    ...
    def test_benchmark_action(self):
        ...
        with (
            ...
            patch.object(
                sys,
                "argv",
                ["da", "benchmark", "--benchmark", "bird_dev", "--datasource", "ns"],
            ),
        ):
            result = main()
    ...
    def test_eval_action(self):
        ...
        with (
            ...
            patch.object(
                sys,
                "argv",
                ["da", "eval", "--datasource", "ns", "--benchmark", "bird_dev"],
            ),
        ):
            result = main()
...
class TestMainPlatformDocAction:
    def test_platform_doc_action(self):
        ...
        with (
            ...
            patch("da.main.setup_exception_handler"),
            patch("da.main.load_agent_config", return_value=mock_config),
            patch("da.agent.agent.bootstrap_platform_doc", mock_bootstrap),
            patch.dict(
                "sys.modules",
                {"da.agent.agent": __import__("da.agent.agent", fromlist=["bootstrap_platform_doc"])},
            ),
            patch.object(sys, "argv", ["da", "platform-doc"]),
        ):
            # Patch the local import inside main()
            import da.agent.agent as _agent_mod
    ...
    def test_generate_dataset_action(self):
        ...
        with (
            ...
            patch.object(
                sys,
                "argv",
                ["da", "generate-dataset", "--dataset_name", "ds1"],
            ),
        ):
            result = main()
