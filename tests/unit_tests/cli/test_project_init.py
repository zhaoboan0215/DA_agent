...
"""Unit tests for da/cli/project_init.py.
...
from da.cli.project_init import run_project_init
from da.configuration.project_config import load_project_override
...
class TestRunProjectInit:
    def test_happy_path_writes_all_three_fields(self, tmp_path):
        ...
        with (
            patch("da.cli.project_init.select_choice", side_effect=["deepseek", "db2"]),
            patch("da.cli.project_init.Prompt.ask", return_value="my_proj"),
        ):
    ...
    def test_uses_base_target_as_default(self, tmp_path):
        ...
        with (
            patch("da.cli.project_init.select_choice", side_effect=["", ""]),
            patch("da.cli.project_init.Prompt.ask", return_value="_my_cwd_"),
        ):
            override = run_project_init(base, cwd=str(tmp_path))
        assert override.target == "deepseek"
    ...
    def test_invalid_project_name_reprompts(self, tmp_path):
        ...
        with (
            patch("da.cli.project_init.select_choice", side_effect=["openai", "db1"]),
            # First attempt has forbidden space; second is valid
            patch("da.cli.project_init.Prompt.ask", side_effect=["bad name with spaces", "good_name"]),
        ):
            override = run_project_init(base, cwd=str(tmp_path))
    ...
    def test_default_project_name_derived_from_cwd(self, tmp_path):
        ...
        with (
            patch("da.cli.project_init.select_choice", side_effect=["openai", "db1"]),
            patch("da.cli.project_init.Prompt.ask", return_value=expected_default),
        ):
            override = run_project_init(base, cwd=str(tmp_path))
