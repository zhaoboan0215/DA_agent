...
"""Unit tests for ``DA.cli.service_commands`` dispatcher."""
...
class TestPreflightMissingAdapter:
    ...
    generic ``_build_adapter`` path raises a cryptic ``DaException`` deep
...
class TestAsyncExecution:
    ...
    def test_run_async_never_touches_shared_bg_loop(self):
        """Service calls must NOT be scheduled on ``DaCLI._bg_loop``.
