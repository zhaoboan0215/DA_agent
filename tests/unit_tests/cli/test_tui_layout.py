...
"""Regression guards for the Da TUI layout.
...
from da.cli.tui.app import DaApp


def _build_app() -> DaApp:
    return DaApp(status_tokens_fn=lambda: [], dispatch_fn=lambda _: None)


class TestCompletionsMenuWired:
    def test_completions_menu_wraps_completions_menu_control(self):
        """DaApp inlines prompt_toolkit's ``CompletionsMenu`` layout — a
    ...
    def test_menu_sits_between_input_and_bottom_separator(self):
        ...
        # DaApp wraps the HSplit directly; grab the children list.
