...
from da.cli.interactive_init import (
    InteractiveInit,
...
class TestInit:
    ...
    def test_llm_config_probe_success(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch.dict("sys.modules", {"da.models.openai_model": mock_module}):
    ...
    def test_config_file_generation(self, monkeypatch):
        """N4-03: agent.yml + project-level .da/config.yml round-trip."""
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            from da.configuration.project_config import ProjectTarget
            ...
            project_cfg_path = Path(tmpdir) / ".DA" / "config.yml"
            assert project_cfg_path.exists(), "Project-level .da/config.yml should be created"
    ...
    def test_optional_component_init(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("da.cli.interactive_init.Confirm.ask") as mock_confirm:
            ...
            with (
                patch("da.cli.interactive_init.Confirm.ask") as mock_confirm,
                patch("da.cli.interactive_init.init_metadata_and_log_result") as mock_metadata,
            ):
            ...
            with (
                patch("da.cli.interactive_init.Confirm.ask") as mock_confirm,
                patch("da.cli.interactive_init.Prompt.ask") as mock_prompt,
                patch("da.cli.interactive_init.init_metadata_and_log_result") as mock_metadata,
                patch("da.cli.interactive_init.overwrite_sql_and_log_result") as mock_sql,
            ):
...
class TestReferenceSqlStreamHandler:
    ...
    def test_task_started_does_nothing(self):
        from da.schemas.batch_events import BatchStage
...
class TestConfigureWorkspace:
    def test_success_creates_directory(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("da.cli.interactive_init.Prompt.ask", return_value=workspace):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch("da.cli.interactive_init.Prompt.ask", return_value=workspace):
                with patch("pathlib.Path.mkdir", side_effect=PermissionError("denied")):
                    with patch("da.cli.interactive_init.print_rich_exception") as mock_print_exc:
                        result = init._configure_workspace()
...
class TestDisplayMethods:
    ...
    def test_display_completion_smoke(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "da" in init.console.file.getvalue().lower()
...
class TestConfigureLLM:
    def test_empty_api_key_returns_false(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("da.cli.interactive_init.select_choice", return_value="openai"),
                patch("da.cli.interactive_init.getpass", return_value=""),
            ):
    ...
    def test_api_key_provider_writes_provider_level_credentials(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("da.cli.interactive_init.select_choice", side_effect=["kimi", "kimi-k2.5"]),
                patch("da.cli.interactive_init.Prompt.ask", return_value="https://api.moonshot.cn/v1"),
                patch("da.cli.interactive_init.getpass", return_value="test-key"),
                patch.dict("sys.modules", {"da.models.kimi_model": mock_module}),
            ):
                result = init._configure_llm()
    ...
    def test_api_key_provider_probe_failure_drops_pending_target(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("da.cli.interactive_init.select_choice", side_effect=["openai", "gpt-4.1"]),
                patch("da.cli.interactive_init.Prompt.ask", return_value="https://api.openai.com/v1"),
                patch("da.cli.interactive_init.getpass", return_value="test-key"),
                patch.dict("sys.modules", {"da.models.openai_model": mock_module}),
            ):
                result = init._configure_llm()
...
class TestOverwriteSqlAndLogResult:
    def test_exception_is_caught_and_printed(self):
        ...
        with (
            patch(
                "da.configuration.agent_config_loader.load_agent_config", side_effect=RuntimeError("config error")
            ),
            patch("da.cli.interactive_init.print_rich_exception") as mock_print_exc,
        ):
            overwrite_sql_and_log_result(
...
class TestConfigureLLMSubscriptionOAuth:
    ...
    def test_subscription_success_writes_provider_section(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("da.cli.interactive_init.select_choice", return_value="claude_subscription"),
                patch("da.cli.interactive_init.configure_claude_subscription", return_value=helper_return),
            ):
    ...
    def test_oauth_helper_failure_returns_false(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with (
                patch("da.cli.interactive_init.select_choice", return_value="codex"),
                patch("da.cli.interactive_init.configure_codex_oauth", return_value=None),
