...
"""Unit tests for da/cli/provider_model_catalog.py."""
...
from da.cli import provider_model_catalog as pmc
...
@pytest.fixture
def fake_DA_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect `_cache_file_path()` to a tmp DA_home."""
    pm = MagicMock()
    pm.DA_home = tmp_path
...
class TestCacheIO:
    def test_load_cached_models_returns_none_when_file_missing(self, fake_DA_home: Path) -> None:
    ...
    def test_save_then_load_roundtrip_v2_details(self, fake_DA_home: Path) -> None:
        buckets = {
            "openai": [
                {
                    "id": "gpt-4o",
        ...
        cache_path = fake_DA_home / "cache" / pmc.CACHE_FILE_NAME
    ...
    def test_save_is_atomic_no_leftover_tmp(self, fake_DA_home: Path) -> None:
        pmc.save_cached_models({"openai": [{"id": "gpt-4o"}]})
        tmp = fake_DA_home / "cache" / (pmc.CACHE_FILE_NAME + ".tmp")
        assert not tmp.exists()

    def test_cache_file_contains_version_and_fetched_at(self, fake_DA_home: Path) -> None:
        pmc.save_cached_models({"openai": [{"id": "gpt-4o", "context_length": 128000}]})
        raw = json.loads((fake_DA_home / "cache" / pmc.CACHE_FILE_NAME).read_text())
    ...
    def test_load_v1_cache_is_backward_compatible(self, fake_DA_home: Path) -> None:
        """v1 caches (flat slug lists) are upgraded on read to avoid a cold cache."""
        _write_cache(fake_DA_home, {"openai": ["gpt-4o", "gpt-4.1"]}, version=1)

        assert pmc.load_cached_models() == {"openai": ["gpt-4o", "gpt-4.1"]}
        assert pmc.load_cached_model_details() == {"openai": [{"id": "gpt-4o"}, {"id": "gpt-4.1"}]}

    def test_load_rejects_wrong_version(self, fake_DA_home: Path) -> None:
        _write_cache(fake_DA_home, {"openai": [{"id": "gpt-4o"}]}, version=99)
        assert pmc.load_cached_models() is None
        assert pmc.load_cached_model_details() is None

    def test_load_rejects_missing_models_key(self, fake_DA_home: Path) -> None:
        cache = fake_DA_home / "cache" / pmc.CACHE_FILE_NAME
        cache.parent.mkdir(parents=True, exist_ok=True)
        cache.write_text(json.dumps({"version": 2, "source": "openrouter"}))
        assert pmc.load_cached_models() is None
        assert pmc.load_cached_model_details() is None

    def test_load_rejects_corrupt_json(self, fake_DA_home: Path) -> None:
        cache = fake_DA_home / "cache" / pmc.CACHE_FILE_NAME
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_load_filters_non_string_values(self, fake_DA_home: Path) -> None:
        _write_cache(
            fake_DA_home,
    ...
    def test_load_mixed_v2_ignores_invalid_entries(self, fake_DA_home: Path) -> None:
        _write_cache(
            fake_DA_home,
    ...
    def test_save_swallows_os_error(self, fake_DA_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        def _boom(*_a: Any, **_kw: Any) -> None:
            raise OSError("disk full")

        monkeypatch.setattr(Path, "write_text", _boom)
        pmc.save_cached_models({"openai": [{"id": "gpt-4o"}]})
        assert not (fake_DA_home / "cache" / pmc.CACHE_FILE_NAME).exists()
...
class TestResolveProviderModels:
    def test_remote_success_overlays_models_and_writes_cache(self, fake_DA_home: Path) -> None:
        def handler(_req: httpx.Request) -> httpx.Response:
            return httpx.Response(
                200,
                json=_payload(
    ...
    def test_preserves_non_models_fields(self, fake_DA_home: Path) -> None:
        def handler(_req: httpx.Request) -> httpx.Response:
    ...
    def test_protected_providers_keep_local_models(self, fake_DA_home: Path) -> None:
    ...
    def test_cache_written_is_v2_with_metadata(self, fake_DA_home: Path) -> None:
    ...
    def test_falls_back_to_cache_when_remote_fails(self, fake_DA_home: Path) -> None:
        _write_cache(fake_DA_home, {"openai": [{"id": "cached-gpt-x"}]}, version=2)
    ...
    def test_cache_fallback_drops_vendors_absent_from_providers_yml(self, fake_DA_home: Path) -> None:
    ...
    def test_falls_back_to_local_when_no_cache_and_remote_fails(self, fake_DA_home: Path) -> None:
        transport = _raising_transport(httpx.ConnectError("offline"))
        original = httpx.Client
    ...
    def test_falls_back_to_local_on_corrupt_cache(self, fake_DA_home: Path) -> None:
        cache = fake_DA_home / "cache" / pmc.CACHE_FILE_NAME
        cache.parent.mkdir(parents=True, exist_ok=True)
    ...
    def test_remote_empty_provider_keeps_local(self, fake_DA_home: Path) -> None:
    ...
    def test_does_not_mutate_input_catalog(self, fake_DA_home: Path) -> None:
        def handler(_req: httpx.Request) -> httpx.Response:
            return httpx.Response(200, json=_payload("openai/gpt-5.2"))
...
class TestSilenceContract:
    def test_fetch_timeout_only_emits_debug(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        caplog.set_level(logging.DEBUG, logger="da.cli.provider_model_catalog")
        with patch.object(pmc.httpx, "Client", side_effect=_factory):
            assert pmc.fetch_openrouter_models() is None
    ...
    def test_resolve_on_triple_failure_only_emits_debug(
        self, fake_DA_home: Path, caplog: pytest.LogCaptureFixture
    ) -> None:
        transport = _raising_transport(httpx.ConnectError("offline"))
        original = httpx.Client
...
class TestResolveModelsFrom:
    ...
    def test_resolve_after_remote_overlay(self, fake_DA_home: Path) -> None:
        def handler(_req: httpx.Request) -> httpx.Response:
    ...
    def test_l3_fallback_still_resolves_models_from(self, fake_DA_home: Path) -> None:
        transport = _raising_transport(httpx.ConnectError("offline"))
        original = httpx.Client
