...
"""Unit tests for :mod:`DA.cli.tui.app`.
...
from da.cli.tui.app import EXIT_SENTINEL, DaApp, tui_enabled


@pytest.fixture
def tui_app() -> DaApp:
    """Construct a minimal :class:`DaApp` wired to recording callbacks."""
    status_calls: list = []

    def _status() -> list:
        status_calls.append(True)
        return [("class:status-bar", "DA")]
...
class TestDaAppState:
    def test_agent_running_is_fresh_threading_event(self, tui_app: DaApp) -> None:
        assert isinstance(tui_app.agent_running, threading.Event)
        assert tui_app.agent_running.is_set() is False

    def test_submit_blank_input_is_rejected(self, tui_app: DaApp) -> None:
    ...
    def test_submit_while_running_is_swallowed(self, tui_app: DaApp) -> None:
    ...
    def test_submit_without_loop_runs_synchronously(self, tui_app: DaApp) -> None:
...
class TestOnDispatchDone:
    def test_clears_running_flag_on_success(self, tui_app: DaApp) -> None:
        tui_app.agent_running.set()
    ...
    def test_clears_running_flag_on_exception(self, tui_app: DaApp) -> None:
        tui_app.agent_running.set()
        future: Future = Future()
    ...
    def test_exit_sentinel_triggers_exit(self, tui_app: DaApp) -> None:
        tui_app.agent_running.set()
        future: Future = Future()
    ...
    def test_non_exit_result_does_not_call_exit(self, tui_app: DaApp) -> None:
        future: Future = Future()
...
class TestStatusTokens:
    def test_status_tokens_are_wrapped_in_formatted_text(self, tui_app: DaApp) -> None:
        ...
        # plumbing.
        assert list(ft) == [("class:status-bar", "DA")]
...
class TestInputPrompt:
    def test_prompt_uses_busy_style_when_running(self, tui_app: DaApp) -> None:
        tui_app.agent_running.set()
        rendered = tui_app._get_input_prompt()
        assert rendered == [("class:input-prompt.busy", "> ")]

    def test_prompt_uses_idle_style_when_not_running(self, tui_app: DaApp) -> None:
        assert tui_app.agent_running.is_set() is False
        rendered = tui_app._get_input_prompt()
...
class TestKeyBindingsContract:
    ...
    @staticmethod
    def _enter_handler(app: DaApp):
        from prompt_toolkit.keys import Keys

        for binding in app.key_bindings.bindings:
            if Keys.ControlM in getattr(binding, "keys", ()):
                return binding.handler
        raise AssertionError("DaApp must register an Enter binding")

    def test_enter_swallowed_while_running(self, tui_app: DaApp) -> None:
    ...
    def test_enter_dispatches_when_idle(self, tui_app: DaApp) -> None:
        handler = self._enter_handler(tui_app)
    ...
    def test_enter_applies_active_completion_and_submits(self, tui_app: DaApp) -> None:
    ...
    def test_enter_closes_menu_without_highlight_and_submits(self, tui_app: DaApp) -> None:
...
def test_exit_when_loop_absent_is_noop(tui_app: DaApp) -> None:
...
def test_invalidate_without_loop_is_noop(tui_app: DaApp) -> None:
...
def test_env_var_whitespace_is_ignored(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # operators commonly paste ``DA_TUI= 0`` with a stray space.
    monkeypatch.setenv("DA_TUI", "  0  ")
    assert tui_enabled() is False
...
def test_os_environ_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("DA_TUI", raising=False)
    assert "DA_TUI" not in os.environ


def _binding_for_key(app: DaApp, key):
    for binding in app.key_bindings.bindings:
...
class TestCtrlDBinding:
    ...
    def test_exits_when_buffer_empty(self, tui_app: DaApp) -> None:
        from prompt_toolkit.keys import Keys
    ...
    def test_noop_when_buffer_has_text(self, tui_app: DaApp) -> None:
        from prompt_toolkit.keys import Keys
...
class TestCtrlCBinding:
    ...
    behavior is wired by DaCLI (tested separately)."""

    def test_clears_buffer_when_idle(self, tui_app: DaApp) -> None:
        from prompt_toolkit.keys import Keys
    ...
    def test_noop_when_agent_running(self, tui_app: DaApp) -> None:
        from prompt_toolkit.keys import Keys
        ...
        handler(event)
        # When the agent is running, the default handler is inert: DaCLI
        # installs a more specific c-c binding that routes to the node's
        # interrupt_controller.
        event.app.current_buffer.reset.assert_not_called()


def test_set_input_text_replaces_buffer(tui_app: DaApp) -> None:
...
def test_safe_dispatch_reraises_system_exit(tui_app: DaApp) -> None:
...
def test_safe_dispatch_logs_and_returns_none_on_base_exception(tui_app: DaApp) -> None:
    def _explode(text: str):
