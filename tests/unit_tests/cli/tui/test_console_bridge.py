...
"""Unit tests for :mod:`DA.cli.tui.console_bridge`.
...
def test_schedules_via_run_in_terminal_when_app_active() -> None:
    ...
    with (
        mock.patch("da.cli.tui.console_bridge.get_app_or_none", return_value=fake_app),
        mock.patch("da.cli.tui.console_bridge.run_in_terminal", return_value=fake_future) as mocked_rit,
    ):
        fn = mock.MagicMock()
