...
"""
Unit tests for da/cli/repl.py.

Tests cover:
- CommandType enum
- DaCLI._parse_command: EXIT, TOOL, SLASH, CHAT, SQL, legacy-prefix UNKNOWN
- DaCLI.check_agent_available: ready / initializing / not ready
- DatasourceCommands._switch: same datasource, switch to different
- DaCLI._smart_display_table: empty data, few columns, many columns
- DaCLI._get_prompt_text: normal/plan mode
- DaCLI.create_combined_completer: returns a completer

DaCLI is instantiated via a fully mocked __init__ to avoid prompt_toolkit/
...
from da.cli.repl import CommandType, DaCLI

# ---------------------------------------------------------------------------
# Factory: build a minimal DaCLI without running __init__
# ---------------------------------------------------------------------------


def _make_cli(agent_config, available_subagents=None):
    """Create a DaCLI instance with __init__ bypassed.

    All attributes that the tested methods rely on are set directly.
    """
    cli = object.__new__(DaCLI)
    ...
    from da.schemas.action_history import ActionHistoryManager
    ...
    # slash names wired by ``DaCLI._build_slash_handler_map``.
    cli.commands = {
...
class TestParseCommand:
    ...
    def test_sql_trailing_semicolon_stripped(self, cli):
        ...
        with patch("da.cli.repl.parse_sql_type") as mock_parse:
    ...
    def test_parse_sql_exception_falls_back_to_chat(self, cli):
        ...
        with patch("da.cli.repl.parse_sql_type", side_effect=Exception("parse error")):
...
class TestExecuteSql:
    ...
    def test_content_set_sql_updates_cli_context_in_place(self, cli):
        ...
        with patch("da.cli.repl.parse_sql_type", return_value=SQLType.CONTENT_SET):
    ...
    def test_non_content_set_sql_does_not_update_context(self, cli):
        ...
        with patch("da.cli.repl.parse_sql_type", return_value=SQLType.DDL):
...
class TestCreateCombinedCompleterExtended:
    def test_returns_a_completer(self, cli):
        from da.cli.autocomplete import AtReferenceCompleter, SlashCommandCompleter
...
class TestBuildBannerPanel:
    ...
    def test_narrow_terminal_falls_back_to_text(self, real_agent_config):
        ...
        assert f"DA v{__version__}" in output
...
class TestCmdAgent:
    ...
    def test_cmd_agent_no_args_interactive(self, cli):
        ...
        with patch("da.cli.repl.ListSelectorApp") as mock_cls:
