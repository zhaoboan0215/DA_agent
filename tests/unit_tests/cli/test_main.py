...
"""
Unit tests for da/cli/main.py — ArgumentParser, Application, main().

All external dependencies (DaCLI, run_web_interface, configure_logging) are mocked.
"""
...
from da.cli.main import Application, ArgumentParser
...
class TestArgumentParser:
    ...
    def test_parse_args_debug_flag(self):
        ap = ArgumentParser()
        with patch.object(sys, "argv", ["da", "--debug", "--datasource", "ns1"]):
            args = ap.parse_args()
    ...
    def test_parse_args_print(self):
        ap = ArgumentParser()
        with patch.object(sys, "argv", ["da", "--datasource", "ns1", "--print", "hello"]):
            args = ap.parse_args()
        assert args.print_mode == "hello"

    def test_parse_args_print_short(self):
        ap = ArgumentParser()
        with patch.object(sys, "argv", ["da", "--datasource", "ns1", "-p", "hello"]):
            args = ap.parse_args()
        assert args.print_mode == "hello"

    def test_parse_args_resume(self):
        ap = ArgumentParser()
        with patch.object(sys, "argv", ["da", "--datasource", "ns1", "--print", "hello", "--resume", "sess_123"]):
            args = ap.parse_args()
        assert args.resume == "sess_123"

    def test_parse_args_web(self):
        ap = ArgumentParser()
        with patch.object(sys, "argv", ["da", "--datasource", "ns1", "--web"]):
            args = ap.parse_args()
        assert args.web is True

    def test_print_and_web_are_mutually_exclusive(self):
        ap = ArgumentParser()
        with patch.object(sys, "argv", ["da", "--web", "--print", "hello"]):
...
class TestApplicationRun:
    def test_run_no_datasource_repl_proceeds(self):
        ...
        with (
            ...
            patch("da.cli.main.configure_logging"),
            patch.object(app, "_ensure_project_config"),
            patch.object(app, "_resolve_default_datasource", return_value=""),
            patch("da.cli.main.DaCLI", return_value=mock_cli) as mock_cli_cls,
        ):
            app.run()
    ...
    def test_run_no_datasource_non_repl_exits(self):
        ...
        with (
            ...
            patch("da.cli.main.configure_logging"),
            patch.object(app, "_resolve_default_datasource", return_value=""),
            patch("da.cli.main.DaCLI") as mock_cli_cls,
        ):
            app.run()
    ...
    def test_run_print_mode(self):
        ...
        with (
            ...
            patch("da.cli.print_mode.PrintModeRunner", return_value=mock_runner) as MockRunner,
        ):
            app.run()
    ...
    def test_run_interactive_mode(self):
        ...
        with (
            ...
            patch("da.cli.main.configure_logging"),
            patch.object(app, "_ensure_project_config") as mock_ensure,
            patch("da.cli.main.DaCLI", return_value=mock_cli) as MockCLI,
        ):
            app.run()
...
class TestEnsureProjectConfig:
    def test_creates_minimal_config_when_missing(self):
        ...
        with (
            patch("da.configuration.project_config.project_config_path", return_value=mock_path),
            patch("da.configuration.agent_config_loader.load_agent_config", return_value=mock_base) as mock_load,
            patch("da.configuration.project_config.save_project_override") as mock_save,
        ):
    ...
    def test_idempotent_when_file_exists(self):
        ...
        with (
            patch("da.configuration.project_config.project_config_path", return_value=mock_path),
            patch("da.configuration.agent_config_loader.load_agent_config") as mock_load,
            patch("da.cli.project_init.run_project_init") as mock_wizard,
            patch("da.configuration.project_config.load_project_override", return_value=None),
        ):
            app._ensure_project_config(args)
    ...
    def test_raises_when_base_config_fails(self):
        ...
        with (
            patch("da.configuration.project_config.project_config_path", return_value=mock_path),
            patch(
                "da.configuration.agent_config_loader.load_agent_config",
...
class TestRepairProjectOverrides:
    ...
    def test_returns_when_override_is_none(self):
        ...
        with (
            patch("da.configuration.project_config.load_project_override", return_value=None),
            patch("da.configuration.agent_config_loader.configuration_manager") as mock_cfg_mgr,
            patch("da.configuration.project_config.save_project_override") as mock_save,
        ):
    ...
    def test_skips_when_all_valid(self):
        ...
        with (
            patch("da.configuration.project_config.load_project_override", return_value=override),
            patch(
                "da.configuration.agent_config_loader.configuration_manager",
                return_value=self._mock_mgr(raw),
            ),
            patch("da.cli._cli_utils.select_choice") as mock_pick,
            patch("da.configuration.project_config.save_project_override") as mock_save,
        ):
            app._repair_project_overrides(args)
    ...
    def test_repairs_stale_default_datasource(self):
        ...
        with (
            ...
            patch("da.cli._cli_utils.select_choice", return_value="bench") as mock_pick,
            patch("da.configuration.project_config.save_project_override") as mock_save,
...
class TestResolveDefaultDatabase:
    ...
    def test_returns_service_default_datasource(self):
        ...
        with patch("da.configuration.agent_config_loader.load_agent_config", return_value=config):
    ...
    def test_no_databases_returns_empty(self):
        ...
        with (
            patch("da.configuration.agent_config_loader.load_agent_config", return_value=config),
        ):
            result = app._resolve_default_datasource(args)
        assert result == ""
...
class TestRunWebInterface:
    def test_delegates_to_run_web_interface(self):
        ...
        with patch("da.cli.web.run_web_interface") as mock_web:
            with patch.dict("sys.modules", {"da.cli.web": MagicMock(run_web_interface=mock_web)}):
...
class TestMain:
    def test_main_delegates_to_app_run(self):
        from da.cli.main import main

        with (
            patch.object(sys, "argv", ["da"]),
            patch("da.cli.main.Application") as MockApp,
        ):
            mock_app = MagicMock()
    ...
    def test_main_skill_subcommand(self):
        ...
        with (
            patch.object(sys, "argv", ["da", "skill", "list"]),
            patch("da.cli.main.configure_logging"),
            patch.dict(
                "sys.modules",
                {
                    "da.main": mock_main_mod,
                    "da.cli.skill_cli": mock_skill_cli,
                },
            ),
