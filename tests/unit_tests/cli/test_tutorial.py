...
"""Tests for DA.cli.tutorial module and related metric/reference_sql init functions."""
...
import da.storage.metric.metric_init as metric_init
import da.storage.reference_sql.reference_sql_init as reference_sql_init
from da.cli import tutorial as tutorial_module
from da.cli.tutorial import BenchmarkTutorial, dict_to_yaml_str
...
class TestBenchmarkTutorialEnsureFiles:
    ...
    def test_creates_benchmark_directory_if_not_exists(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "da.cli.interactive_init.copy_data_file",
            mock_copy_data_file,
        )
...
class TestBenchmarkTutorialAddSubAgents:
    ...
    def test_adds_two_sub_agents(self, tmp_path, monkeypatch):
        ...
        # Check first agent (DA_schools)
        first_call = mock_manager.save_agent.call_args_list[0]
        first_config = first_call[0][0]
        assert first_config.system_prompt == "DA_schools"
        assert first_config.tools == "db_tools, date_parsing_tools"
        assert first_call[1]["previous_name"] == "DA_schools"

        # Check second agent (DA_schools_context)
        second_call = mock_manager.save_agent.call_args_list[1]
        second_config = second_call[0][0]
        assert second_config.system_prompt == "DA_schools_context"
        assert "context_search_tools" in second_config.tools
        assert second_call[1]["previous_name"] == "DA_schools_context"

        output = buffer.getvalue()
        assert "DA_schools" in output
        assert "DA_schools_context" in output
...
class TestBenchmarkTutorialAddWorkflows:
    ...
    def test_adds_workflows(self, tmp_path, monkeypatch):
        ...
        assert "DA_schools" in workflows
        assert "DA_schools_context" in workflows
        assert workflows["DA_schools"] == ["DA_schools", "execute_sql", "output"]
        assert workflows["DA_schools_context"] == ["DA_schools_context", "execute_sql", "output"]
...
class TestBenchmarkTutorialRun:
    ...
    def test_run_completes_successfully(self, tmp_path, monkeypatch):
        ...
        mock_copy_data_file = MagicMock()
        monkeypatch.setattr("da.cli.interactive_init.copy_data_file", mock_copy_data_file)

        mock_init_metadata = MagicMock()
        mock_init_sql = MagicMock()
        monkeypatch.setattr("da.cli.interactive_init.init_metadata_and_log_result", mock_init_metadata)
        monkeypatch.setattr("da.cli.interactive_init.overwrite_sql_and_log_result", mock_init_sql)
        ...
        assert result == 0
        output = buffer.getvalue()
        assert "Welcome to Da benchmark" in output
...
class TestBenchmarkTutorialInitMetrics:
    ...
    def test_init_metrics_success(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "da.storage.metric.metric_init.init_success_story_metrics",
            mock_init_metrics,
        )

        # Mock create_vector_connection to avoid creating da_db_ in working directory
        monkeypatch.setattr("da.storage.backend_holder.create_vector_connection", MagicMock())
