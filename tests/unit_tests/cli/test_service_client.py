...
"""Unit tests for ``DA.cli.service_client``: registry + allow-list."""
...
from da.cli.service_client import READ_METHODS, ServiceClient, ServiceClientRegistry
...
class TestServiceClientRegistry:
    ...
    def test_discover_lists_all_service_types(self):
        ...
        with patch.dict("da.cli.service_client._PROBES", always_available):
    ...
    def test_lazy_construction_then_active_status(self):
        ...
        with (
            patch.dict("da.cli.service_client._FACTORIES", {"bi_platforms": factory}),
            patch.dict("da.cli.service_client._PROBES", {"bi_platforms": lambda c, n: True}),
        ):
            registry = ServiceClientRegistry(cfg)
            # Not built yet.
    ...
    def test_case_insensitive_lookup(self):
        cfg = _fake_agent_config(bi_platforms={"Superset": {}})
        factory = MagicMock(return_value=_FakeBITool())
        with patch.dict("da.cli.service_client._FACTORIES", {"bi_platforms": factory}):
            registry = ServiceClientRegistry(cfg)
    ...
    def test_scheduler_factory_wired(self):
        ...
        with patch.dict("da.cli.service_client._FACTORIES", {"schedulers": factory}):
            registry = ServiceClientRegistry(cfg)
    ...
    def test_semantic_factory_wired(self):
        ...
        with patch.dict("da.cli.service_client._FACTORIES", {"semantic_layer": factory}):
            registry = ServiceClientRegistry(cfg)
    ...
    def test_list_services_drops_to_configured_after_invalidation(self):
        ...
        with (
            patch.dict(
                "da.cli.service_client._FACTORIES",
    ...
    def test_missing_adapter_status_beats_active_even_with_cached_client(self):
        ...
        """
        cfg = _fake_agent_config(bi_platforms={"broken": {}})
        with patch.dict("da.cli.service_client._PROBES", {"bi_platforms": lambda c, n: False}):
            registry = ServiceClientRegistry(cfg)
    ...
    def test_adapter_available_is_cached_until_fingerprint_change(self):
        ...
        with patch.dict("da.cli.service_client._PROBES", {"bi_platforms": probe}):
            registry = ServiceClientRegistry(cfg)
...
class TestAdapterProbes:
    ...
    def test_bi_probe_returns_false_when_datus_bi_core_missing(self):
        from da.cli.service_client import _probe_bi_adapter
    ...
    def test_scheduler_probe_needs_core_package_importable(self):
        from da.cli.service_client import _probe_scheduler_adapter

        # Force ImportError on the ``DA_scheduler_core.registry`` submodule
        ...
        with patch.dict(
            "sys.modules",
            {"DA_scheduler_core": None, "DA_scheduler_core.registry": None},
        ):
    ...
    def test_scheduler_probe_checks_platform_registration(self):
        ...
        ``DA-scheduler-core`` is typically present; the real-world gap is
        the platform-specific package (e.g. ``DA-scheduler-airflow``) that
        ...
        with patch.dict(
            "sys.modules",
            {
                "DA_scheduler_core": core_module,
                "DA_scheduler_core.registry": core_module.registry,
            },
        ):
    ...
    def test_probe_helper_defensive_on_exception(self):
        from da.cli.service_client import _probe

        with patch.dict(
            "da.cli.service_client._PROBES",
