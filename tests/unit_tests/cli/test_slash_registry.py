...
"""Integrity tests for ``DA.cli.slash_registry``.

These tests protect the invariants consumed by :class:`DaCLI`:
...
- the registry matches the handler map wired in ``DaCLI``

Registry integrity is a pure-data concern, so no ``DaCLI`` construction is
needed here.
"""

from __future__ import annotations

from da.cli.slash_registry import (
    GROUP_ORDER,
    GROUP_TITLES,
    SLASH_COMMANDS,
    all_tokens,
...
class TestHandlerMapAlignment:
    """Handlers are wired in :meth:`DaCLI._build_slash_handler_map`.
    ...
    def test_handler_map_covers_every_canonical_name(self):
        ...
        source = inspect.getsource(DaCLI._build_slash_handler_map)
        for spec in SLASH_COMMANDS:
            needle = f'"{spec.name}"'
