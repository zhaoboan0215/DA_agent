...
from da.cli.datasource_app import DatasourceApp, DatasourceSelection, _View
...
class TestInstallPlugin:
    def test_install_success_loads_adapter(self):
        ...
        with (
            ...
            result = cmds._install_plugin("DA-mysql")
            assert result is True
            mock_invalidate.assert_called_once()
            mock_import.assert_called_once_with("DA_mysql")
    ...
    def test_install_failure(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            result = cmds._install_plugin("DA-postgresql")
...
class TestMenuLoop:
    ...
    def test_menu_needs_install_then_continues(self):
        ...
        with (
            ...
            cmds._run_menu()
            mock_install.assert_called_once_with("DA-snowflake")
...
class TestTestConnectivity:
    def test_success(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("da.cli.datasource_commands.detect_db_connectivity", return_value=(True, "")):
            assert cmds._test_connectivity("test", {"type": "duckdb"}) is True

    def test_failure(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("da.cli.datasource_commands.detect_db_connectivity", return_value=(False, "refused")):
...
class TestSave:
    def test_save_calls_configuration_manager(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("da.configuration.agent_config_loader.configuration_manager") as mock_mgr:
    ...
    def test_save_returns_false_on_exception(self):
        cli = _make_cli()
        cmds = DatasourceCommands(cli)
        with patch("da.configuration.agent_config_loader.configuration_manager", side_effect=Exception("boom")):
            result = cmds._save()
            assert result is False
...
class TestDatasourceAppViews:
    ...
    def test_enter_type_select(self):
        ...
        app = DatasourceApp(cli.agent_config, MagicMock())
        with patch(
            "da.tools.db_tools.connector_registry.list_available_adapters",
