...
"""
Unit tests for da/cli/generation_hooks.py — GenerationHooks.
...
from da.cli.execution_state import InteractionCancelled
from da.cli.generation_hooks import (
    GenerationCancelledException,
    GenerationHooks,
    normalize_kb_relative_path,
    resolve_kb_sandbox_path,
)
from da.tools.func_tool.filesystem_tools import FilesystemFuncTool  # noqa: F401
...
class TestSyncToStorage:
    ...
    async def test_semantic_type_calls_sync_semantic(self, hooks):
        ...
        with patch("da.cli.generation_hooks.GenerationHooks._sync_semantic_to_db", return_value=mock_result):
    ...
    async def test_sql_summary_type_calls_sync_reference(self, hooks):
        ...
        with patch("da.cli.generation_hooks.GenerationHooks._sync_reference_sql_to_db", return_value=mock_result):
    ...
    async def test_ext_knowledge_type_calls_sync(self, hooks):
        ...
        with patch("da.cli.generation_hooks.GenerationHooks._sync_ext_knowledge_to_db", return_value=mock_result):
    ...
    async def test_exception_returns_error_string(self, hooks):
        with patch(
            "da.cli.generation_hooks.GenerationHooks._sync_semantic_to_db",
...
class TestSyncReferenceSqlToDb:
    def test_valid_template_yaml(self, tmp_path):
        ...
        from da.cli.generation_hooks import GenerationHooks
        ...
        with (
            patch("da.cli.generation_hooks.ReferenceSqlRAG") as mock_rag_cls,
            patch(
                "da.storage.reference_sql.init_utils.exists_reference_sql",
                return_value=set(),
            ),
            patch(
                "da.storage.reference_sql.init_utils.gen_reference_sql_id",
                return_value="new_id",
            ),
        ):
    ...
    def test_duplicate_skipped(self, tmp_path):
        ...
        mock_config = MagicMock()
        with (
            patch("da.cli.generation_hooks.ReferenceSqlRAG"),
            patch(
                "da.storage.reference_sql.init_utils.exists_reference_sql",
...
class TestSyncReferenceTemplateToDb:
    def test_valid_template_yaml(self, tmp_path):
        ...
        with (
            patch("da.storage.reference_template.store.ReferenceTemplateRAG") as mock_rag_cls,
            patch(
                "da.storage.reference_template.init_utils.exists_reference_templates",
                return_value=set(),
            ),
            patch(
                "da.storage.reference_template.init_utils.gen_reference_template_id",
                return_value="new_tpl_id",
            ),
            patch(
                "da.storage.reference_template.template_file_processor.extract_template_parameters",
    ...
    def test_duplicate_skipped(self, tmp_path):
        ...
        mock_config = MagicMock()
        with (
            patch("da.storage.reference_template.store.ReferenceTemplateRAG"),
            patch(
                "da.storage.reference_template.init_utils.exists_reference_templates",
...
class TestSyncSemanticToDbBooleanCoercion:
    ...
    def test_boolean_fields_are_coerced(self, agent_config, create_metric_val, is_partition_val, tmp_path):
        ...
        with (
            patch("da.cli.generation_hooks.SemanticModelRAG", return_value=mock_semantic_rag),
            patch("da.cli.generation_hooks.MetricRAG", return_value=mock_metric_rag),
        ):
...
class TestSyncSemanticToDbMetricOnlyDiagnostic:
    ...
    def test_metric_only_sync_with_no_metric_blocks_returns_actionable_error(self, agent_config, tmp_path):
        ...
        with (
            patch("da.cli.generation_hooks.SemanticModelRAG"),
            patch("da.cli.generation_hooks.MetricRAG"),
        ):
            result = GenerationHooks._sync_semantic_to_db(
...
class TestResolvePathCommonpathValueError:
    def test_returns_empty_when_commonpath_raises_value_error(self, broker):
        ...
        h = GenerationHooks(broker=broker, agent_config=cfg)
        with patch("da.cli.generation_hooks.os.path.commonpath", side_effect=ValueError("mixed drives")):
