...
"""
Unit tests for da/cli/print_mode.py
"""
...
from da.schemas.message_content import MessageContent, MessagePayload
...
def _make_runner(**overrides):
    ...
    with (
        patch("da.cli.print_mode.load_agent_config") as mock_cfg,
        patch("da.cli.print_mode.AtReferenceCompleter") as mock_completer,
    ):
...
class TestWritePayload:
    def test_writes_json_line(self):
        with (
            patch("da.cli.print_mode.load_agent_config") as mock_cfg,
            patch("da.cli.print_mode.AtReferenceCompleter"),
        ):
            mock_cfg.return_value = MagicMock(datasource_configs={})
            from da.cli.print_mode import PrintModeRunner
...
class TestResumeSessionId:
    def test_resume_sets_session_id(self):
        ...
        with (
            patch("da.cli.print_mode.create_interactive_node", return_value=mock_node),
            patch("da.cli.print_mode.create_node_input", return_value=MagicMock()),
            patch("da.models.session_manager.SessionManager", return_value=mock_session_mgr),
        ):
            runner.run()
    ...
    def test_resume_subagent_name_from_args(self):
        ...
        with (
            patch("da.cli.print_mode.create_interactive_node", return_value=mock_node) as mock_create_node,
            patch("da.cli.print_mode.create_node_input", return_value=MagicMock()),
            patch("da.models.session_manager.SessionManager", return_value=mock_session_mgr),
        ):
            runner.run()
...
class TestRunUsesFactory:
    def test_run_calls_factory(self):
        ...
        with (
            patch("da.cli.print_mode.create_interactive_node", return_value=mock_node) as mock_create_node,
            patch("da.cli.print_mode.create_node_input", return_value=mock_input) as mock_create_input,
        ):
            runner.run()
...
class TestRunProxyTools:
    def test_run_applies_proxy_tools_when_patterns_set(self):
        ...
        with (
            patch("da.cli.print_mode.create_interactive_node", return_value=mock_node),
            patch("da.cli.print_mode.create_node_input", return_value=MagicMock()),
            patch("da.tools.proxy.proxy_tool.apply_proxy_tools") as mock_apply,
        ):
            runner.run()
