...
"""Unit tests for da/cli/service_manager.py"""
...
class TestServiceManagerInit:
    ...
    def test_init_with_valid_config_sets_agent_config(self, tmp_path):
        ...
        with (
            patch("da.cli.service_manager.load_agent_config", return_value=mock_config),
            patch("da.cli.service_manager.console"),
        ):
            from da.cli.service_manager import ServiceManager
    ...
    def test_init_with_missing_file_prints_error(self, tmp_path):
        ...
        exc = DaException(ErrorCode.COMMON_FILE_NOT_FOUND, message_args={"config_name": "agent", "file_name": "x"})
        with (
            patch("da.cli.service_manager.load_agent_config", side_effect=exc),
            patch("da.cli.service_manager.console") as mock_console,
        ):
            from da.cli.service_manager import ServiceManager
    ...
    def test_init_with_generic_exception_sets_agent_config_none(self):
        ...
        with (
            patch("da.cli.service_manager.load_agent_config", side_effect=RuntimeError("unexpected")),
            patch("da.cli.service_manager.console") as mock_console,
        ):
            from da.cli.service_manager import ServiceManager
    ...
    def test_init_with_other_DA_exception_prints_message(self):
        """When load_agent_config raises a non-FILE_NOT_FOUND DaException, message is printed."""
        exc = DaException(ErrorCode.COMMON_CONFIG_ERROR, message_args={})
        with (
            patch("da.cli.service_manager.load_agent_config", side_effect=exc),
            patch("da.cli.service_manager.console") as mock_console,
        ):
...
class TestServiceManagerRun:
    ...
    def _make_sm(self, mock_config):
        with patch("da.cli.service_manager.load_agent_config", return_value=mock_config):
            from da.cli.service_manager import ServiceManager
    ...
    def test_run_returns_1_when_no_config(self):
        ...
        with (
            patch("da.cli.service_manager.load_agent_config", side_effect=RuntimeError("fail")),
            patch("da.cli.service_manager.console"),
        ):
            from da.cli.service_manager import ServiceManager
    ...
    def test_run_unknown_command_returns_1(self):
        ...
        mock_config = _make_agent_config({})
        sm = self._make_sm(mock_config)
        with patch("da.cli.service_manager.console") as mock_console:
...
class TestServiceManagerAdd:
    ...
    def test_add_invalid_db_name_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.Prompt.ask", return_value="bad name"),
            patch("da.cli.service_manager.console"),
        ):
            result = sm.add()
        assert result == 1
    ...
    def test_add_empty_name_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.Prompt.ask", return_value="   "),
            patch("da.cli.service_manager.console"),
        ):
            result = sm.add()
        assert result == 1
    ...
    def test_add_duplicate_name_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.Prompt.ask", return_value="existing_db"),
            patch("da.cli.service_manager.console") as mock_console,
        ):
            result = sm.add()
        assert result == 1
    ...
    def test_add_no_adapters_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.Prompt.ask", return_value="new_db"),
            patch("da.cli.service_manager.console") as mock_console,
            patch("da.tools.db_tools.connector_registry", mock_registry),
        ):
            result = sm.add()
        assert result == 1
        calls = [str(c) for c in mock_console.print.call_args_list]
    ...
    def test_add_no_config_fields_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.Prompt.ask", side_effect=lambda *a, **kw: next(prompt_values)),
            patch("da.cli.service_manager.console") as mock_console,
            patch("da.tools.db_tools.connector_registry", mock_registry),
        ):
            result = sm.add()
    ...
    def test_add_connectivity_failure_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.Prompt.ask", side_effect=lambda *a, **kw: next(prompt_values)),
            patch("da.cli.service_manager.Confirm.ask", return_value=False),
            patch("da.cli.service_manager.console") as mock_console,
            patch("da.tools.db_tools.connector_registry", mock_registry),
            patch("da.cli.service_manager.detect_db_connectivity", return_value=(False, "connection refused")),
        ):
            result = sm.add()
        assert result == 1
        calls = [str(c) for c in mock_console.print.call_args_list]
    ...
    def test_add_successful_save_returns_0(self):
        ...
        from da.configuration.agent_config import DbConfig
        ...
        with (
            ...
            patch("da.cli.service_manager.console"),
            patch("da.tools.db_tools.connector_registry", mock_registry),
            patch("da.cli.service_manager.detect_db_connectivity", return_value=(True, "")),
            patch("da.cli.service_manager.DbConfig.filter_kwargs", return_value=fake_db_config),
    ...
    def test_add_first_db_auto_set_as_default(self):
        ...
        with (
            ...
            patch("da.tools.db_tools.connector_registry", mock_registry),
            patch("da.cli.service_manager.detect_db_connectivity", return_value=(True, "")),
            patch("da.cli.service_manager.DbConfig.filter_kwargs", side_effect=fake_filter_kwargs),
            patch.object(sm, "_save_configuration", return_value=True),
        ):
            result = sm.add()
        assert result == 0
    ...
    def test_add_subsequent_db_prompts_for_default(self):
        ...
        with (
            patch("da.cli.service_manager.Prompt.ask", side_effect=lambda *a, **kw: next(prompt_values)),
            patch("da.cli.service_manager.Confirm.ask", side_effect=lambda *a, **kw: next(confirm_values)),
            patch("da.cli.service_manager.console"),
            patch("da.tools.db_tools.connector_registry", mock_registry),
            patch("da.cli.service_manager.detect_db_connectivity", return_value=(True, "")),
            patch("da.cli.service_manager.DbConfig.filter_kwargs", return_value=fake_db_config),
    ...
    def test_add_password_field_uses_getpass(self):
        ...
        with (
            ...
            patch("da.cli.service_manager.console"),
            patch("da.tools.db_tools.connector_registry", mock_registry),
            patch("da.cli.service_manager.getpass", return_value=getpass_value) as mock_getpass,
            patch("da.cli.service_manager.detect_db_connectivity", return_value=(True, "")),
            patch("da.cli.service_manager.DbConfig.filter_kwargs", return_value=fake_db_config),
            patch.object(sm, "_save_configuration", return_value=True),
        ):
    ...
    def test_add_file_path_field_with_sample_file(self):
        ...
        with (
            ...
            patch("da.cli.service_manager.console"),
            patch("da.tools.db_tools.connector_registry", mock_registry),
            patch("da.cli.service_manager.get_path_manager", return_value=mock_path_manager),
            patch("da.cli.service_manager.detect_db_connectivity", return_value=(True, "")),
            patch("da.cli.service_manager.DbConfig.filter_kwargs", return_value=fake_db_config),
            patch.object(sm, "_save_configuration", return_value=True),
        ):
...
class TestServiceManagerDelete:
    ...
    def test_delete_nonexistent_db_name_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.load_agent_config", return_value=mock_config),
            patch("da.cli.service_manager.console") as mock_console,
            patch("da.cli.service_manager.Prompt.ask", return_value="nonexistent_db"),
        ):
            from da.cli.service_manager import ServiceManager
    ...
    def test_delete_cancelled_by_user_returns_1(self):
        ...
        with (
            patch("da.cli.service_manager.load_agent_config", return_value=mock_config),
            patch("da.cli.service_manager.console"),
            patch("da.cli.service_manager.Prompt.ask", return_value="my_db"),
            patch("da.cli.service_manager.Confirm.ask", return_value=False),
        ):
            from da.cli.service_manager import ServiceManager
    ...
    def test_delete_confirmed_saves_and_returns_0(self):
        ...
        with (
            ...
            patch("da.cli.service_manager.console") as mock_console,
            patch("da.cli.service_manager.Prompt.ask", return_value="my_db"),
            patch("da.cli.service_manager.Confirm.ask", return_value=True),
        ):
            from da.cli.service_manager import ServiceManager
            ...
            with patch("da.cli.service_manager.configuration_manager", return_value=mock_cm):
                ret = sm.delete()
            assert ret == 0
            calls = [str(c) for c in mock_console.print.call_args_list]
    ...
    def test_delete_confirmed_save_failure_returns_1(self):
        ...
        with (
            ...
            patch("da.cli.service_manager.Prompt.ask", return_value="my_db"),
            patch("da.cli.service_manager.Confirm.ask", return_value=True),
            patch("da.cli.service_manager.configuration_manager", side_effect=RuntimeError("disk error")),
        ):
            from da.cli.service_manager import ServiceManager
...
class TestServiceManagerSaveConfiguration:
    ...
    def test_save_configuration_builds_correct_structure(self):
        ...
        with (
            patch("da.cli.service_manager.load_agent_config", return_value=mock_config),
            patch("da.cli.service_manager.configuration_manager", return_value=mock_cm),
            patch("da.cli.service_manager.console"),
        ):
            from da.cli.service_manager import ServiceManager
    ...
    def test_save_configuration_returns_false_on_exception(self):
        ...
        with (
            patch("da.cli.service_manager.load_agent_config", return_value=mock_config),
            patch("da.cli.service_manager.configuration_manager", side_effect=RuntimeError("write error")),
            patch("da.cli.service_manager.console"),
        ):
            from da.cli.service_manager import ServiceManager
...
class TestValidateDbName:
    ...
    def test_valid_name_returns_true(self):
        from da.cli.service_manager import _validate_db_name
...
class TestValidatePort:
    ...
    def test_valid_port_returns_true(self):
        from da.cli.service_manager import _validate_port
