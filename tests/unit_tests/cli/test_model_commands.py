...
"""Unit tests for ``DA.cli.model_commands.ModelCommands``.
...
from da.cli.model_app import ModelSelection
from da.cli.model_commands import ModelCommands
from da.configuration.agent_config import ProviderConfig
...
class _FakeCli:
    """Minimal stand-in for :class:`DaCLI` used by :class:`ModelCommands`."""
...
class TestInteractiveMenu:
    def test_provider_model_selection_persists_active_target(self, commands):
        mc, cli = commands
        with patch(
            "da.cli.model_commands.ModelApp",
    ...
    def test_provider_only_argument_seeds_picker(self, commands):
        ...
        with patch("da.cli.model_commands.ModelApp", return_value=fake_app) as ma:
            mc.cmd_model("openai")
...
class TestOauthHandoff:
    def test_needs_oauth_runs_configure_then_reopens_with_seed(self, commands):
        ...
        with (
            ...
            patch(
                "da.cli.provider_auth_flows.configure_codex_oauth",
    ...
    def test_needs_oauth_aborts_when_helper_fails(self, commands):
        ...
        with (
            patch("da.cli.model_commands.ModelApp", return_value=fake_app),
            patch(
                "da.cli.provider_auth_flows.configure_codex_oauth",
                return_value=None,
            ),
