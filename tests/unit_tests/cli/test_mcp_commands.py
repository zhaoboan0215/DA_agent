...
"""
Unit tests for da/cli/mcp_commands.py — MCPCommands.
...
def cmd():
    cli = _make_cli()
    with patch("da.cli.mcp_commands.MCPTool") as MockTool:
        mock_tool = MagicMock()
...
class TestCmdMcpList:
    ...
    def test_list_fallback_table_on_screen_error(self, cmd):
        ...
        with patch("da.cli.mcp_commands.MCPServerApp") as MockApp:
...
class TestCmdMcpAdd:
    def test_add_success(self, cmd):
        ...
        with patch("da.cli.mcp_commands.parse_command_string", return_value=("stdio", "myserver", {})):
    ...
    def test_add_exception(self, cmd):
        with patch("da.cli.mcp_commands.parse_command_string", side_effect=ValueError("bad args")):
