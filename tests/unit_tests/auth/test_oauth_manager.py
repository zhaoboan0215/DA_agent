...
class TestGetAccessToken:
    def test_raises_when_not_authenticated(self, manager):
        with pytest.raises(DaException, match="Not authenticated"):
...
class TestRefreshTokens:
    @patch("da.auth.oauth_manager.httpx.post")
    ...
    def test_raises_without_refresh_token(self, manager):
        ...
        with pytest.raises(DaException, match="No refresh token"):
            manager.refresh_tokens()


class TestLoginBrowser:
    @patch("da.auth.oauth_manager.webbrowser.open")
    @patch("da.auth.oauth_manager.HTTPServer")
...
class TestLoginBrowserFull:
    ...
    @patch("da.auth.oauth_manager.HTTPServer")
    @patch.object(OAuthManager, "_exchange_code")
    @patch("da.auth.oauth_manager.generate_pkce_pair")
    @patch("da.auth.oauth_manager.generate_state")
    ...
    @patch("da.auth.oauth_manager.time.monotonic")
    @patch("da.auth.oauth_manager.webbrowser.open")
    @patch("da.auth.oauth_manager.HTTPServer")
    @patch("da.auth.oauth_manager.generate_pkce_pair")
    @patch("da.auth.oauth_manager.generate_state")
    def test_browser_flow_error(self, mock_state, mock_pkce, mock_server_cls, mock_browser, mock_monotonic, manager):
        ...
        with pytest.raises(DaException, match="No authorization code"):
            manager.login_browser()


class TestRefreshTokensErrors:
    @patch("da.auth.oauth_manager.httpx.post")
    def test_http_error_raises_DA_exception(self, mock_post, manager):
        ...
        with pytest.raises(DaException, match="Token refresh failed"):
            manager.refresh_tokens()

    @patch("da.auth.oauth_manager.httpx.post")
    def test_timeout_raises_DA_exception(self, mock_post, manager):
        manager.token_storage.save({"access_token": "old", "refresh_token": "rt"})
        mock_post.side_effect = httpx.TimeoutException("timeout")

        with pytest.raises(DaException, match="timed out"):
            manager.refresh_tokens()
...
class TestExchangeCodeErrors:
    ...
    def test_http_error_raises_DA_exception(self, mock_post, manager):
        ...
        with pytest.raises(DaException, match="Code exchange failed"):
...
class TestHttpTimeout:
    ...
    def test_refresh_passes_timeout(self, mock_post, manager):
        from da.auth.oauth_config import HTTP_TIMEOUT
