...
from da.utils.resource_utils import copy_data_file, do_copy_data_file, package_data_path, read_data_file_text


class TestPackageDataPath:
    def test_returns_path_when_sys_prefix_exists(self, tmp_path):
        resource = "config/test.yml"
        target = tmp_path / "da" / "config" / "test.yml"
...
class TestReadDataFileText:
    ...
    def test_raises_when_file_not_found(self):
        ...
        with pytest.raises(DaException):
            read_data_file_text("nonexistent/path/file.txt", package="da")
