...
"""
Unit tests for da/models/claude_model.py.
...
from da.models.claude_model import ClaudeModel, convert_tools_for_anthropic, wrap_prompt_cache
...
def _make_claude_model(model_config=None):
    ...
    with (
        patch("da.models.openai_compatible.setup_tracing"),
        patch("da.models.openai_compatible.LiteLLMAdapter", return_value=mock_litellm_adapter),
...
class TestClaudeModelInit:
    ...
    def test_anthropic_client_initialized(self):
        with (
            patch("da.models.openai_compatible.setup_tracing"),
            patch("da.models.openai_compatible.LiteLLMAdapter"),
...
class TestClaudeModelGenerate:
    def test_litellm_path_calls_super(self):
        model = _make_claude_model()
        with patch(
            "da.models.openai_compatible.OpenAICompatibleModel.generate", return_value="from litellm"
        ) as mock_super:
    ...
    def test_litellm_path_passes_top_p_none(self):
        ...
        with patch("da.models.openai_compatible.OpenAICompatibleModel.generate", capture_generate):
...
class TestClaudeModelClose:
    ...
    def test_close_handles_exception_gracefully(self):
        ...
        with patch("da.models.claude_model.logger") as mock_logger:
...
class TestClaudeModelSubscriptionAuth:
    def test_subscription_auth_calls_credential_resolver(self):
        ...
        with (
            patch("da.models.openai_compatible.setup_tracing"),
            patch("da.models.openai_compatible.LiteLLMAdapter") as mock_adapter_cls,
            patch("anthropic.Anthropic"),
            patch(
                "da.auth.claude_credential.get_claude_subscription_token",
...
class TestDiagnoseOAuth401:
    ...
    def test_expired_token_raises_expired_error(self, tmp_path):
        ...
        original_error = Exception("401 Unauthorized")
        with patch("da.models.claude_model.Path.home", return_value=tmp_path):
            with pytest.raises(DaException) as exc_info:
                model._diagnose_oauth_401(original_error)
...
class TestGenerateWithMcpStream:
    ...
    async def test_no_tool_calls_yields_final_action(self):
        ...
        from da.schemas.action_history import ActionHistoryManager, ActionRole, ActionStatus
        ...
        actions = []
        with patch("da.models.claude_model.multiple_mcp_servers") as mock_mcp:
...
class TestGenerateWithMcpToolRouting:
    ...
    async def test_duplicate_tool_name_logs_warning(self):
        ...
        with (
            patch("da.models.claude_model.multiple_mcp_servers", side_effect=mock_mcp_ctx),
            patch("da.models.claude_model.convert_tools_for_anthropic", return_value=[]),
            patch("da.models.claude_model.logger") as mock_logger,
        ):
