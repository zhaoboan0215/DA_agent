...
@pytest.fixture
def mock_oauth():
    with patch("da.models.codex_model.OAuthManager") as mock_cls:
...
class TestCodexModelInit:
    def test_init(self, model_config, mock_oauth):
        from da.models.codex_model import CodexModel
    ...
    def test_default_constants_present(self, model_config, mock_oauth):
        ...
        from da.models.codex_model import _CODEX_DEFAULT_CONTEXT_LENGTH, _CODEX_DEFAULT_MAX_TOKENS
...
class TestCodexModelGenerate:
    @patch("da.models.codex_model.OAuthManager")
...
class TestCodexModelUtils:
    ...
    @patch("da.models.codex_model.OAuthManager")
    @patch("da.cli.provider_model_catalog.load_cached_model_details", return_value=None)
...
class TestCodexModelGenerateNonAuthError:
    ...
    def test_generate_reraises_non_auth_error(self, mock_oauth_cls, model_config):
        ...
        with pytest.raises(DaException, match="Codex generate failed"):
            model.generate("test")
...
class TestCodexModelGenerateWithTools:
    @pytest.mark.asyncio
    @patch("da.models.codex_model.OAuthManager")
    @patch("da.models.codex_model.multiple_mcp_servers")
    @patch("da.models.codex_model.Runner")
    @patch("da.models.codex_model.Agent")
    @patch("da.models.codex_model.extract_sql_contexts")
    ...
    async def test_generate_with_tools_max_turns_exceeded(
        ...
        with patch("agents.models.openai_responses.OpenAIResponsesModel"):
            ...
            with pytest.raises(DaException, match="Maximum turns"):
    ...
    async def test_generate_with_tools_with_mcp_and_tools(
        ...
        with (
            ...
            patch("da.models.codex_model.extract_sql_contexts", return_value=[]),
        ):
...
class TestCodexModelBaseUrl:
    ...
    def test_base_url_defaults_when_not_set(self, mock_oauth_cls):
        from da.models.codex_model import CODEX_API_BASE_URL, CodexModel
...
class TestCodexModelJsonOutput401Retry:
    ...
    def test_json_output_reraises_non_auth_error(self, mock_oauth_cls, model_config):
        ...
        with pytest.raises(DaException, match="Codex generate_with_json_output failed"):
...
class TestCodexModelToolsAuth401Retry:
    ...
    async def test_generate_with_tools_401_retry(
        ...
        with patch("agents.models.openai_responses.OpenAIResponsesModel"):
            ...
            # Instead refreshes token and raises DaException.
            from da.utils.exceptions import DaException

            with pytest.raises(DaException, match="Authentication failed"):
