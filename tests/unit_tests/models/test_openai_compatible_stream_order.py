...
async def _collect_actions(events_list) -> List[ActionHistory]:
    ...
    with patch("da.models.openai_compatible.Runner") as mock_runner:
        mock_runner.run_streamed.return_value = fake_result

        # Patch Agent constructor
        with patch("da.models.openai_compatible.Agent"):
            # Patch multiple_mcp_servers context manager
            with patch("da.models.openai_compatible.multiple_mcp_servers") as mock_mcp:
