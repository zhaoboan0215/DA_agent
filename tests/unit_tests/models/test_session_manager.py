...
"""
Unit tests for da/models/session_manager.py.
...
from da.models.session_manager import (
    SessionManager,
...
class TestSessionManagerPathManagerInjection:
    def test_uses_explicit_path_manager_when_session_dir_missing(self, tmp_path):
        path_manager = DaPathManager(tmp_path / "tenant_home", project_name="tenant")
...
class TestGetDetailedUsage:
    ...
    def test_invalid_scope_raises_DA_exception(self, tmp_path, bad_scope):
        """Scope values with special characters raise DaException."""
        with pytest.raises(DaException, match="Invalid scope"):
