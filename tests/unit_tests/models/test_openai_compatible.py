...
"""
Unit tests for da/models/openai_compatible.py.
...
from da.models.openai_compatible import OpenAICompatibleModel, classify_openai_compatible_error
...
class TestGenerate:
    ...
    def test_basic_generate_returns_content(self):
        ...
        with patch("da.models.openai_compatible.litellm.completion", return_value=mock_resp):
    ...
    def test_generate_with_list_prompt(self):
        ...
        with patch("da.models.openai_compatible.litellm.completion", return_value=mock_resp) as mock_lit:
...
class TestWithRetry:
    ...
    def test_raises_da_exception_on_non_retryable_api_error(self):
        model = _make_model()
...
class TestWithRetryAsync:
    ...
    @pytest.mark.asyncio
    async def test_non_retryable_api_error_raises_da_exception(self):
        model = _make_model()
...
class TestModelSpecsOpenRouterCacheMerge:
    ...
    def _clear_specs_cache(self):
        ...
        import da.models.openai_compatible as oc
    ...
    def test_cache_context_length_used_when_yaml_has_no_entry(self):
        ...
        with patch(
            "da.cli.provider_model_catalog.load_cached_model_details",
...
class TestTokenCount:
    def test_returns_litellm_count(self):
        model = _make_model()
        with patch("da.models.openai_compatible.litellm.token_counter", return_value=42):
            count = model.token_count("hello world")
        assert count == 42

    def test_falls_back_to_approximation_on_error(self):
        model = _make_model()
        with patch("da.models.openai_compatible.litellm.token_counter", side_effect=Exception("fail")):
            count = model.token_count("hello world")
...
class TestSaveLlmTrace:
    ...
    def test_handles_write_error_gracefully(self, tmp_path):
        ...
        with (
            ...
            patch("da.models.openai_compatible.logger") as mock_logger,
        ):
            # Should not raise
            model._save_llm_trace("prompt", "response")
...
class TestBuildAgent:
    ...
    def _call_build_agent(self, model, **kwargs):
        ...
        with patch("da.models.openai_compatible.Agent") as MockAgent:
