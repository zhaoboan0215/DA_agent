...
"""
Unit tests for da/api/service.py — DaAPIService and helper functions.
...
from da.api.legacy_models import FeedbackRequest, FeedbackStatus, RunWorkflowRequest
from da.api.service import DaAPIService
...
def _make_service(args=None):
    return DaAPIService(args=args or _make_args())
...
# ---------------------------------------------------------------------------
# DaAPIService.__init__
# ---------------------------------------------------------------------------


class TestDaAPIServiceInit:
...
class TestDaAPIServiceInitialize:
    @pytest.mark.asyncio
    async def test_initialize_sets_config_and_task_store(self):
        ...
        with patch("da.api.service.load_agent_config", return_value=mock_cfg):
            with patch("da.api.service.TaskStore", return_value=mock_task_store):
...
class TestGetAgent:
    ...
    def test_creates_agent_for_new_datasource(self):
        ...
        with patch("da.api.service.Agent", return_value=mock_agent):
...
class TestHealthCheck:
    ...
    async def test_unhealthy_on_exception(self):
        ...
        with patch("da.api.service.Agent", side_effect=RuntimeError("config error")):
            response = await service.health_check()
