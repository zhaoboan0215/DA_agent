...
"""Unit tests for da/api/routes/visualization_routes.py — CI level, zero external deps."""
...
from da.api.routes.visualization_routes import router
...
def _client_with(generate_return):
    ...
    app = _make_app()
    app.dependency_overrides[get_da_service] = lambda: _mock_svc(generate_return)
...
class TestServiceDelegation:
    ...
    def test_passes_all_params_to_service(self, valid_payload):
        ...
        app = _make_app()
        app.dependency_overrides[get_da_service] = lambda: svc
