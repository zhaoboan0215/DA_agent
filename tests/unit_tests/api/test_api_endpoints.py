...
from da.api.legacy_models import (
    FeedbackRequest,
    FeedbackResponse,
    FeedbackStatus,
    HealthResponse,
...
def app(mock_agent_config):
    ...
    with (
        patch("da.api.service.load_agent_config", return_value=mock_agent_config),
        patch("da.api.service.TaskStore", mock_task_store_cls),
        patch("da.api.service.Agent", mock_agent_cls),
    ):
...
def auth_token():
    ...
    from da.api.legacy_auth import auth_service

    token_data = auth_service.generate_access_token("da_client")
