"""Unit tests for da.api.main — merged FastAPI server entry point.
...
class TestAPIArgumentParser:
    def test_defaults(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("DA_DATASOURCE", None)
            os.environ.pop("DA_OUTPUT_DIR", None)
            os.environ.pop("DA_LOG_LEVEL", None)
    ...
    def test_datasource_from_env(self):
        with patch.dict(os.environ, {"DA_DATASOURCE": "staging"}, clear=False):
            args = _build_parser().parse_args([])
        assert args.datasource == "staging"

    def test_output_dir_from_env(self):
        with patch.dict(os.environ, {"DA_OUTPUT_DIR": "/custom/out"}):
            args = _build_parser().parse_args([])
...
class TestStop:
    ...
    def test_stop_running_process_sends_sigterm(self, tmp_path):
        ...
        with (
            patch("da.api.main.os.kill", side_effect=fake_kill),
            patch("da.api.main._is_process_running", side_effect=[True, True, False]),
        ):
    ...
    def test_stop_removes_pid_when_process_lookup_error(self, tmp_path):
        ...
        with patch("da.api.main.os.kill", side_effect=fake_kill):
...
class TestRedirectStdio:
    def test_redirect_creates_log_dir(self, tmp_path):
        ...
        with patch("da.api.main.os.dup2") as mock_dup2:
...
class TestDefaultPaths:
    def test_returns_pid_and_log_paths(self, tmp_path):
        with (
            patch(
                "da.configuration.agent_config_loader.get_agent_home",
                return_value=str(tmp_path),
            ),
            patch("da.utils.path_manager.DaPathManager") as mock_pm_cls,
        ):
            mock_pm = mock_pm_cls.return_value
            mock_pm.pid_file_path.return_value = tmp_path / "da-agent-api.pid"
            mock_pm.logs_dir = tmp_path / "logs"
            pid_file, log_file = _default_paths()
        assert isinstance(pid_file, Path)
        assert isinstance(log_file, Path)
        assert log_file.name == "da-agent-api.log"
...
class TestMainDispatch:
    def test_status_action_exits(self, tmp_path):
        ...
        with (
            patch("da.api.main._default_paths", return_value=(pid_file, log_file)),
            patch("da.api.main.configure_logging"),
            patch.object(sys, "argv", ["da-api", "--action", "status"]),
        ):
            from da.api.main import main
    ...
    def test_stop_action_exits_zero(self, tmp_path):
        ...
        with (
            patch("da.api.main._default_paths", return_value=(pid_file, log_file)),
            patch("da.api.main.configure_logging"),
            patch.object(sys, "argv", ["da-api", "--action", "stop"]),
        ):
            from da.api.main import main
    ...
    def test_main_config_not_found_exits(self, tmp_path):
        ...
        with (
            patch("da.api.main._default_paths", return_value=(tmp_path / "p.pid", tmp_path / "p.log")),
            patch("da.api.main.configure_logging"),
            patch("da.api.main.parse_config_path", side_effect=FileNotFoundError("no such file")),
            patch.object(sys, "argv", ["da-api", "--config", "/nonexistent/agent.yml"]),
        ):
            with pytest.raises(SystemExit) as exc_info:
                main()
    ...
    def test_main_sets_env_vars_and_runs_server(self, tmp_path):
        ...
        with (
            patch("da.api.main._default_paths", return_value=(tmp_path / "p.pid", tmp_path / "p.log")),
            patch("da.api.main.configure_logging"),
            patch("da.api.main.parse_config_path", return_value="/tmp/agent.yml"),
            patch("da.api.main._run_server", side_effect=fake_run_server),
            patch.object(
                sys,
                "argv",
                [
        ...
        assert os.environ.get("DA_DATASOURCE") == "test_main_ns"
        assert os.environ.get("DA_CONFIG") == "/tmp/agent.yml"
        assert captured == {
            "host": "0.0.0.0",
...
class TestStopForceKill:
    def test_stop_force_kills_when_sigterm_ignored(self, tmp_path):
        ...
        with (
            patch("da.api.main.os.kill", side_effect=fake_kill),
            patch("da.api.main._is_process_running", return_value=True),
            patch("da.api.main.time.sleep"),
        ):
...
class TestRunServer:
    def test_reload_uses_import_string(self):
        ...
        with (
            patch("da.api.service.create_app") as mock_create,
            patch("da.api.main.uvicorn.run") as mock_run,
        ):
            _run_server(args, argparse.Namespace())
        mock_create.assert_not_called()
        kwargs = mock_run.call_args.kwargs
        assert mock_run.call_args.args[0] == "da.api.service:app"
    ...
    def test_single_worker_creates_app_instance(self):
        ...
        sentinel_app = object()
        mock_server = patch("da.api.main.uvicorn.Server")
        mock_config = patch("da.api.main.uvicorn.Config")
        with (
            patch("da.api.service.create_app", return_value=sentinel_app) as mock_create,
            mock_config as patched_config,
            mock_server as patched_server,
            patch("da.api.main.asyncio.run") as mock_asyncio_run,
        ):
...
class TestDaemonWorker:
    def test_daemon_worker_writes_pid_and_runs_server(self, tmp_path):
        ...
        with (
            patch("da.api.main.os.setsid") as mock_setsid,
            patch("da.api.main.os.umask") as mock_umask,
            patch("da.api.main.configure_logging") as mock_conf,
            patch("da.api.main._redirect_stdio") as mock_redir,
            patch("da.api.main._run_server") as mock_run,
            patch("da.api.main.atexit.register") as mock_atexit,
            patch("da.api.main.signal.signal") as mock_signal,
        ):
...
class TestMainExtraBranches:
    def test_debug_flag_sets_log_level(self, tmp_path):
        ...
        with (
            patch("da.api.main._default_paths", return_value=(tmp_path / "p.pid", tmp_path / "p.log")),
            patch("da.api.main.configure_logging"),
            patch("da.api.main.parse_config_path", return_value="/tmp/a.yml"),
            patch("da.api.main._run_server", side_effect=fake_run_server),
            patch.object(sys, "argv", ["da-api", "--debug"]),
        ):
            main()
    ...
    def test_restart_action_invokes_stop_then_start(self, tmp_path):
        ...
        with (
            ...
            patch("da.api.main.configure_logging"),
            patch("da.api.main.parse_config_path", return_value="/tmp/a.yml"),
            patch("da.api.main._stop") as mock_stop,
            patch("da.api.main._run_server") as mock_run,
            patch.object(sys, "argv", ["da-api", "--action", "restart"]),
        ):
            main()
    ...
    def test_daemon_with_reload_exits_two(self, tmp_path):
        ...
        with (
            ...
            patch("da.api.main.configure_logging"),
            patch("da.api.main.parse_config_path", return_value="/tmp/a.yml"),
            patch.object(sys, "argv", ["da-api", "--daemon", "--reload"]),
        ):
    ...
    def test_daemon_already_running_exits_zero(self, tmp_path):
        ...
        with (
            patch("da.api.main._default_paths", return_value=(pid_file, tmp_path / "p.log")),
            patch("da.api.main.configure_logging"),
            patch("da.api.main.parse_config_path", return_value="/tmp/a.yml"),
            patch("da.api.main._is_process_running", return_value=True),
            patch("da.api.main.multiprocessing.Process") as mock_proc,
            patch.object(sys, "argv", ["da-api", "--daemon"]),
        ):
            with pytest.raises(SystemExit) as exc:
                main()
    ...
    def test_daemon_spawns_successfully(self, tmp_path):
        ...
        with (
            ...
            patch("da.api.main.parse_config_path", return_value="/tmp/a.yml"),
            patch("da.api.main.time.sleep"),
            patch("da.api.main.multiprocessing.Process", FakeProcess),
            patch("da.api.main.os._exit", side_effect=SystemExit(0)) as mock_exit,
            patch.object(sys, "argv", ["da-api", "--daemon"]),
        ):
            with pytest.raises(SystemExit):
                main()
    ...
    def test_daemon_spawn_failure_exits_one(self, tmp_path):
        ...
        with (
            ...
            patch("da.api.main.parse_config_path", return_value="/tmp/a.yml"),
            patch("da.api.main.time.sleep"),
            patch("da.api.main.multiprocessing.Process", DeadProcess),
            patch("da.api.main.os._exit", side_effect=SystemExit(1)) as mock_exit,
            patch.object(sys, "argv", ["da-api", "--daemon"]),
        ):
            with pytest.raises(SystemExit):
                main()
