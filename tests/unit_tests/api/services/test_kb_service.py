"""Tests for DA.api.services.kb_service — knowledge base bootstrap."""

from datetime import datetime

import pytest

from da.api.models.kb_models import BootstrapDocInput, BootstrapKbEvent, BootstrapKbInput
...
class TestKbServiceBootstrapDocStream:
    ...
    async def test_init_platform_docs_failure_yields_task_failed(self, real_agent_config):
        ...
        from da.storage.document.doc_init import InitResult
        ...
        with patch("da.api.services.kb_service.KbService._run_doc_init", return_value=failed_result):
            events = []
...
class TestKbServiceRunDocInitCancelBridge:
    ...
    async def test_cancel_bridge_propagates_to_init_platform_docs(self, real_agent_config):
        ...
        with patch("da.storage.document.doc_init.init_platform_docs", side_effect=_capture_cancel):
