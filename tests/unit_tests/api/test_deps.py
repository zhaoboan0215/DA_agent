"""Tests for DA.api.deps — dependency injection module."""

from unittest.mock import AsyncMock, MagicMock

import pytest

from da.api import deps
from da.api.auth.context import AppContext
from da.api.deps import get_da_service, init_deps
...
class TestInitDeps:
    ...
    def test_sets_auth_provider(self):
        ...
        mock_cache = MagicMock(spec=DaServiceCache)
...
@pytest.mark.asyncio
class TestGetDaService:
    """Tests for get_da_service — FastAPI dependency."""
    ...
    async def test_authenticates_and_returns_service(self):
        ...
        request.state = MagicMock()
        with patch(
            "da.api.deps.DaService.compute_fingerprint",
            return_value="fp-xyz",
        ) as mock_fp:
            result = await get_da_service(request)
    ...
    async def test_factory_propagates_default_source_and_interactive(self):
        """Factory passes module-level defaults through to DaService constructor."""
        from unittest.mock import patch
        ...
        with (
            patch("da.api.deps.DaService.compute_fingerprint", return_value="fp"),
            patch("da.api.deps.DaService") as mock_svc_cls,
        ):
    ...
    async def test_factory_loads_config_when_none(self, real_agent_config):
        """Factory in get_da_service loads config when ctx.config is None."""
        ...
        try:
            result = await get_da_service(request)
            # If it succeeds, result should be a DaService
            assert isinstance(result, DaService)
