...
class TestGetComparePrompt:
    def test_returns_system_and_user_messages(self):
        from da.prompts.compare_sql_with_mcp import get_compare_prompt
        ...
        with patch("da.prompts.compare_sql_with_mcp.get_prompt_manager") as mock_gpm:
    ...
    def test_get_compare_system_prompt_returns_string(self):
        from da.prompts.compare_sql_with_mcp import get_compare_system_prompt
...
class TestCompareSqlPrompt:
    def test_returns_system_and_user_messages(self):
        from da.prompts.compare_sql import compare_sql_prompt
        ...
        with patch("da.prompts.compare_sql.get_prompt_manager") as mock_gpm:
            mock_pm = mock_gpm.return_value
...
class TestOutputCheckingGenPrompt:
    def test_returns_single_user_message(self):
        from da.prompts.output_checking import gen_prompt

        with patch("da.prompts.output_checking.get_prompt_manager") as mock_gpm:
            mock_pm = mock_gpm.return_value
    ...
    def test_handles_metrics(self):
        from da.prompts.output_checking import gen_prompt
        from da.schemas.node_models import Metric
...
class TestSchemaLineageGenPrompt:
    def test_returns_empty_list_when_no_metadata(self):
        from da.prompts.schema_lineage import gen_prompt
    ...
    def test_returns_system_and_user_messages(self):
        ...
        with patch("da.prompts.schema_lineage.get_prompt_manager") as mock_gpm:
            mock_pm = mock_gpm.return_value
    ...
    def test_gen_summary_prompt_returns_user_message(self):
        from da.prompts.schema_lineage import gen_summary_prompt
...
class TestCreateSelectionPrompt:
    def test_returns_prompt_string(self):
        from da.prompts.selection import create_selection_prompt

        candidates = {"cand_1": {"sql": "SELECT 1", "result": "1"}}

        with patch("da.prompts.selection.get_prompt_manager") as mock_gpm:
            mock_pm = mock_gpm.return_value
...
class TestGenMetricsV12Template:
    ...
    def test_v12_template_renders_without_error(self):
        ...
        from da.prompts.prompt_manager import PromptManager
