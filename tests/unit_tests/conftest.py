...
def _isolate_project_cwd(monkeypatch, tmp_path):
    ...
    1. ``load_project_override`` (see ``agent_config_loader._apply_project_override``)
       reads ``{cwd}/.da/config.yml`` unconditionally. On a developer workstation
...
def reset_global_singletons():
    ...
    """
    from da.utils.path_manager import reset_path_manager
...
def mock_llm_create():
    ...
    with patch("da.models.base.LLMBaseModel.create_model", return_value=mock_model):
        yield mock_model
