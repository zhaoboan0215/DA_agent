...
"""
Additional unit tests for da/configuration/agent_config.py
...
from da.configuration.agent_config import (
    AgentConfig,
    BenchmarkConfig,
    DbConfig,
    DocumentConfig,
...
class TestResolveEnv:
    ...
    def test_missing_env_var_returns_placeholder(self, monkeypatch):
        ...
        monkeypatch.delenv("DA_NONEXISTENT_VAR_XYZ", raising=False)
        result = resolve_env("${DA_NONEXISTENT_VAR_XYZ}")
        assert result == "<MISSING:DA_NONEXISTENT_VAR_XYZ>"
...
class TestAgentConfigServiceSelectors:
    ...
    def test_resolve_semantic_adapter_requires_explicit_choice_for_multiple_entries(self, tmp_path):
        ...
        with pytest.raises(DaException, match="Multiple semantic layers are configured"):
    ...
    def test_default_scheduler_service_rejects_multiple_defaults(self, tmp_path):
        with pytest.raises(DaException, match="Multiple scheduler services are marked"):
            self._make(
                tmp_path,
                services={
                    "datasources": {},
    ...
    def test_get_scheduler_config_requires_explicit_choice_when_multiple_instances_exist(self, tmp_path):
        ...
        with pytest.raises(DaException, match="set `scheduler_service` on the scheduler node"):
    ...
    def test_init_scheduler_services_requires_declared_type(self, tmp_path):
        with pytest.raises(DaException, match="must declare a scheduler `type`"):
            self._make(
                tmp_path,
                services={
                    "datasources": {},
...
class TestNormalizeProjectName:
    ...
    def test_replaces_slashes(self):
        from da.configuration.agent_config import _normalize_project_name
    ...
    def test_long_path_truncated_with_md5(self):
        ...
        from da.configuration.agent_config import _PROJECT_NAME_MAX_LEN, _normalize_project_name
    ...
    def test_derived_name_passes_backend_segment_check(self):
        ...
        from da.configuration.agent_config import _normalize_project_name
        from da.storage.rdb.sqlite_backend import _safe_path_segment
...
class TestAgentConfigProjectLayout:
    ...
    def test_sessions_and_data_sharded_by_project_name(self, tmp_path):
        ...
        DA_home = (tmp_path / "DA_home").resolve()
        # data_dir is the backend root (no project suffix); project sharding
        # is surfaced via project_data_dir and owned by backends.
        assert cfg.path_manager.data_dir == DA_home / "data"
        assert cfg.path_manager.project_data_dir == DA_home / "data" / "demo_project"
        assert cfg.path_manager.sessions_dir == DA_home / "sessions" / "demo_project"
    ...
    def test_overlong_project_name_rejected(self, tmp_path):
        from da.configuration.agent_config import _PROJECT_NAME_MAX_LEN
...
class TestServicesConfigFromDict:
    ...
    def test_legacy_databases_key_is_rejected(self):
        ...
        with pytest.raises(DaException, match="services.databases has been renamed to services.datasources"):
...
class TestProviderConfigurationDispatch:
    ...
    def test_active_model_raises_when_nothing_is_configured(self, tmp_path):
        ...
        assert "da init" in exc_info.value.message
    ...
    def test_set_active_provider_model_writes_project_config(self, tmp_path):
        ...
        project_cfg = tmp_path / ".DA" / "config.yml"
        import yaml
