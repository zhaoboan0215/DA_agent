...
"""
Unit tests for da/configuration/agent_config_loader.py
...
from da.configuration.agent_config_loader import (
...
class TestParseConfigPath:
    ...
    def test_explicit_non_existing_non_default_raises(self):
        with pytest.raises(DaException, match="not found"):
            parse_config_path("nonexistent_config.yml")

    def test_explicit_default_fallback(self, tmp_path, monkeypatch):
        """When config_file is 'conf/agent.yml' and doesn't exist anywhere, raises DaException."""
        # chdir to tmp_path (no conf/agent.yml there) and patch home to nonexistent path
        monkeypatch.chdir(tmp_path)
        with patch("da.configuration.agent_config_loader.Path.home", return_value=tmp_path / "noexist"):
    ...
    def test_home_config_found(self, tmp_path, monkeypatch):
        """Falls back to ~/.da/conf/agent.yml."""
        # Use a directory without local conf
        monkeypatch.chdir(tmp_path)
        home_conf = tmp_path / ".DA" / "conf"
        home_conf.mkdir(parents=True)
        cfg = home_conf / "agent.yml"
        cfg.write_text("agent: {}")

        with patch("da.configuration.agent_config_loader.Path.home", return_value=tmp_path):
            result = parse_config_path("")
        assert result.name == "agent.yml"
...
class TestApplyProjectOverride:
    """_apply_project_override validates & merges ./.da/config.yml into agent_raw."""
    ...
    def test_no_override_is_noop(self):
        ...
        with patch(
            "da.configuration.agent_config_loader.load_project_override",
            return_value=None,
        ):
    ...
    def test_valid_default_datasource_flips_default_flags(self):
        ...
        uniformly across every entry point (REPL, da-api, SDK)."""
        agent_raw = self._base_raw()
        with patch(
            "da.configuration.agent_config_loader.load_project_override",
    ...
    def test_invalid_default_datasource_raises(self):
        ...
        with patch(
            ...
            with pytest.raises(DaException) as exc:
                _apply_project_override(agent_raw)
...
class TestLoadNodeConfig:
    def test_with_model(self):
        ...
        data = {"model": "gpt-4o"}
        with patch("da.configuration.agent_config_loader.NodeType.type_input", return_value={}):
...
class TestLoadAgentConfigResolution:
    ...
    is populated for every entry point (REPL, da-api, da-gateway, SDK),
    ...
    def test_project_overlay_wins_over_base_default(self, tmp_path, reset_global_singletons):
        """``.da/config.yml::default_datasource`` overrides the base default flag."""
        cfg = self._write_base_yaml(
            tmp_path,
            {
    ...
    def test_raises_when_ambiguous_default(self, tmp_path, reset_global_singletons):
        ...
        tells the user how to fix it (run ``DA`` init wizard)."""
        cfg = self._write_base_yaml(
            tmp_path,
            {
